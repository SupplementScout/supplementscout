...
const SEQUENTIAL_APPROVER_IDENTITY_MIGRATION = "20260919210000_fix_sequential_parent_approver_identity.sql";
const SEQUENTIAL_APPROVER_IDENTITY_SHA256 = "8e2235061b90f88a22740f654e0fd87e1a9ffddd9915194dec3ba5ece2fe1659";
...
test("RA-004 interfaces remain SHA-bound and excluded from staging and production deployment", () => {
  ...
  for (const [filename, sha256] of Object.entries(expected)) {
    ...
    assert.equal(sha256File(path.join(SOURCE, filename)), sha256);
...
test("RA-004 activation selects exactly the two approved staging migrations and defers every unrelated pending migration", () => {
  ...
  assert.equal(result.activation_id, "ra004-staging-interfaces-2026-09-25-v1");
...
test("RA-004 activation fails closed for production, target drift, SHA drift and unrelated pending admission", () => {
