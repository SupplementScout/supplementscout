...
test("production records v10 parent support and selects only the 71-sibling policy", () => {
  const contract = CONTRACTS.PRODUCTION;
  assert.deepEqual(contract.pending, [{ filename: TEN_REPS_V10_SIBLING_MIGRATION, sha256: TEN_REPS_V10_SIBLING_SHA256, expectedCatalogueDeltas: {} }]);
...
test("production binds its exact ledger and selects the 10 Reps v10 policies", () => {
  ...
  assert.deepEqual(result.pending_files, [TEN_REPS_V10_SIBLING_MIGRATION]);
  assert.equal(result.pending_file, TEN_REPS_V10_SIBLING_MIGRATION);
  assert.equal(result.pending_sha256, TEN_REPS_V10_SIBLING_SHA256);
  assert.deepEqual(result.pending_sha256s, { [TEN_REPS_V10_SIBLING_MIGRATION]: TEN_REPS_V10_SIBLING_SHA256 });
