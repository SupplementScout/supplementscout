...
- Routine automation is implemented in `.github/workflows/six-pack-offer-refresh.yml`. One shared expandable manifest drives the complete retailer scope; the workflow runs daily at `03:17 UTC` and supports manual dry-run/apply. Every run tests contracts, fetches the 231 exact WooCommerce product pages for all 440 approved variants, validates source identity and full manifest coverage, applies only approved existing mappings through separate production approver/executor roles, takes a fresh source capture, requires idempotency and uploads evidence.
