...
function loadReviewedBaseline(file = MANIFEST_FILE) {
  ...
  return {
    ...
    buildContract({ artifact, targetEnvironment, expiresAt }) {
      ...
      const artifactRows = stableRows(artifact.rows.map((row) => ({ external_product_id: String(row.external_product_id), external_variant_id: String(row.external_variant_id), action: row.action, changed_fields: Object.entries(row.changed_fields).filter(([key, value]) => key !== "blocked" && value).map(([key]) => key).sort(), before: { price: money(row.atomic_plan.expected_state.offer.price), shipping_cost: money(row.atomic_plan.expected_state.offer.shipping_cost), total_price: money(row.atomic_plan.expected_state.offer.total_price), in_stock: Boolean(row.atomic_plan.expected_state.offer.in_stock), url: String(row.atomic_plan.expected_state.offer.url) }, after: { price: money(row.atomic_plan.offer.values.price), shipping_cost: money(row.atomic_plan.offer.values.shipping_cost), total_price: money(row.atomic_plan.offer.values.total_price), in_stock: Boolean(row.atomic_plan.offer.values.in_stock), url: String(row.atomic_plan.offer.values.url) } })));
      if (JSON.stringify(artifactRows) !== JSON.stringify(reviewedRows)) {
        const index = artifactRows.findIndex((row, rowIndex) => JSON.stringify(row) !== JSON.stringify(reviewedRows[rowIndex]));
        throw new Error(`live Simply artifact differs from approved manifest at row ${index}: ${JSON.stringify({ artifact: artifactRows[index], approved: reviewedRows[index] })}`);
      }
