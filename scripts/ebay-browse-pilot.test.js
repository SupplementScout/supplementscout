...
test("Batch O refresh continuity is sealed to the nine owner-reviewed missing-GTIN listings", () => {
  const scopes = new Map(REFRESH_SCOPES.filter((scope) => Number(scope.offer_id) >= 2660 && Number(scope.offer_id) <= 2679).map((scope) => [scope.offer_id, scope]));
...
test("Batch P refresh continuity is sealed to its exact owner-reviewed exceptions", () => {
  const scopes = new Map(REFRESH_SCOPES.filter((scope) => Number(scope.offer_id) >= 2680 && Number(scope.offer_id) <= 2699).map((scope) => [scope.offer_id, scope]));
  const evaluation = (offerId, seller, blockers, reasons, returnedGtin) => {
    const scope = scopes.get(offerId);
    return {
      decision: blockers.length ? "REJECT" : "REVIEW",
...
test("Batch Q refresh continuity is sealed to its eight exact owner-reviewed exceptions", () => {
  const scopes = new Map(REFRESH_SCOPES.filter((scope) => Number(scope.offer_id) >= 2700 && Number(scope.offer_id) <= 2719).map((scope) => [scope.offer_id, scope]));
  const evaluation = (offerId, seller, blockers, reasons, returnedGtin) => {
    const scope = scopes.get(offerId);
    return {
      decision: blockers.length ? "REJECT" : "REVIEW",
  ...
  const missingGtin = [
    ["2705", "gymstop", ["FORMAT_UNPROVEN", "RETURNED_GTIN_UNPROVEN", "SIZE_UNPROVEN"]],
    ["2707", "soovital", ["RETURNED_GTIN_UNPROVEN"]],
    ["2709", "muscle-factory-co-uk", ["FORMAT_UNPROVEN", "RETURNED_GTIN_UNPROVEN", "SIZE_UNPROVEN"]],
    ["2716", "1thetreehouse", ["RETURNED_GTIN_UNPROVEN"]],
    ["2717", "z.m.s.limited", ["RETURNED_GTIN_UNPROVEN"]],
    ["2718", "ccolta", ["RETURNED_GTIN_UNPROVEN"]],
    ["2719", "premium_supps", ["RETURNED_GTIN_UNPROVEN"]],
  ];
  for (const [offerId, seller, reasons] of missingGtin) {
    assert.equal(classifyContinuity(scopes.get(offerId), evaluation(offerId, seller, ["CANONICAL_GTIN_INVALID"], reasons, null)).tier, "sealed_owner_reviewed_missing_gtin_continuity");
  }
  const exactScope = scopes.get("2708");
  const exactReasons = ["FORMAT_UNPROVEN", "SELLER_FEEDBACK_BELOW_PROPOSED_THRESHOLD"];
  assert.equal(classifyContinuity(exactScope, evaluation("2708", "superfoodmarket", [], exactReasons, exactScope.gtin)).tier, "sealed_owner_reviewed_exact_gtin_metadata_continuity");
  assert.equal(classifyContinuity(exactScope, evaluation("2708", "different-seller", [], exactReasons, exactScope.gtin)).eligible, false);
  assert.equal(classifyContinuity(scopes.get("2705"), evaluation("2705", "gymstop", ["CANONICAL_GTIN_INVALID"], ["RETURNED_GTIN_UNPROVEN"], null)).eligible, false);
});
...
test("Batch R refresh continuity remains sealed to the exact approved item, seller and anomalies", () => {
  ...
  const missing = scopes.get("2720");
  assert.equal(classifyContinuity(missing, evaluation("2720", "themadtitansupplements", ["CANONICAL_GTIN_INVALID", "FLAVOUR_MISMATCH"], ["RETURNED_GTIN_UNPROVEN", "SIZE_UNPROVEN"], null)).tier, "sealed_owner_reviewed_missing_gtin_continuity");
  const returned = scopes.get("2731");
  assert.equal(classifyContinuity(returned, evaluation("2731", "fitgamerltd", ["CANONICAL_GTIN_INVALID", "FLAVOUR_MISMATCH", "GTIN_MISMATCH"], [], "5060245603423")).tier, "sealed_owner_reviewed_missing_gtin_continuity");
  const exact = scopes.get("2722");
  assert.equal(classifyContinuity(exact, evaluation("2722", "powerbodyltd", ["BRAND_MISMATCH"], [], exact.gtin)).tier, "sealed_owner_reviewed_exact_gtin_metadata_continuity");
  assert.equal(classifyContinuity(missing, evaluation("2720", "different-seller", ["CANONICAL_GTIN_INVALID", "FLAVOUR_MISMATCH"], ["RETURNED_GTIN_UNPROVEN", "SIZE_UNPROVEN"], null)).eligible, false);
  assert.equal(classifyContinuity(returned, evaluation("2731", "fitgamerltd", ["CANONICAL_GTIN_INVALID", "FLAVOUR_MISMATCH", "GTIN_MISMATCH"], [], "different-gtin")).eligible, false);
...
test("Batch Q owner review seals twenty product-level second-retailer listings after two replacements", () => {
  const review = JSON.parse(fs.readFileSync(path.join(process.cwd(), "docs/rollouts/ebay-offer-canary/batch-q-review.json"), "utf8"));
  const validated = validateBatchQRollout();
  assert.equal(BATCH_Q_CONFIRMATION, "OWNER_APPROVED_EBAY_BATCH_Q_EXACT_20");
  assert.equal(review.owner_approval.approved_count, 20);
  assert.deepEqual(review.owner_approval.rejected_original_review_numbers, [3, 5]);
  assert.deepEqual(review.owner_approval.replacement_final_review_numbers, [3, 5]);
  assert.equal(review.dry_run.plan_count, 20);
  assert.equal(review.dry_run.blocked_row_count, 0);
  assert.equal(review.dry_run.database_writes, 0);
  assert.equal(review.identity_summary.exact_returned_gtin_rows, 13);
  assert.equal(review.identity_summary.owner_reviewed_missing_returned_gtin_rows, 7);
  assert.equal(review.identity_summary.creates_second_retailer_products, 20);
  assert.equal(review.identity_summary.expands_existing_ebay_products, 0);
  assert.equal(review.identity_summary.owner_reviewed_seller_feedback_exceptions, 1);
  assert.equal(review.identity_summary.owner_reviewed_future_expiry_disclosures, 2);
  assert.equal(validated.entries.length, 20);
  assert.equal(BATCH_Q_IDENTITIES.length, 20);
  assert.equal(new Set(BATCH_Q_IDENTITIES).size, 20);
  assert.equal(new Set(validated.entries.map(({ approved }) => approved.product_id)).size, 20);
  assert.equal(parseBatchQArgs(["--mode=preflight", "--output=tmp/batch-q-preflight.json"]).mode, "preflight");
  assert.throws(() => parseBatchQArgs(["--mode=execute", "--output=tmp/batch-q.json"]), /preflight\|validate\|apply/);
});

test("Batch Q live preflight rechecks all twenty exact owner-reviewed item identities", async () => {
  resetTokenCache();
  const { entries } = validateBatchQRollout();
  const fixtures = entries.map(({ approved }) => item({
    itemId: approved.external_variant_id,
    legacyItemId: approved.legacy_item_id,
    title: approved.live_title,
  ...
  const fetchImpl = async (url) => String(url).includes("oauth2/token")
    ? { ok: true, json: async () => ({ access_token: "token", expires_in: 7200 }) }
    : { ok: true, status: 200, json: async () => fixtures.find((entry) => String(url).includes(encodeURIComponent(entry.itemId))) };
  assert.equal((await validateBatchQLiveSources(fetchImpl, config)).length, 20);
  resetTokenCache();
});

test("Batch Q workflow is manual, exact-confirmation guarded and verifies twenty no-ops", () => {
  const workflow = fs.readFileSync(path.join(process.cwd(), ".github/workflows/ebay-offer-batch-q.yml"), "utf8");
