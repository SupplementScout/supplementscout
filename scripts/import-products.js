...
const PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING =
  PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.post_create_remaining_sibling_profile;
const PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING_SHA256 =
...
const TEN_REPS_REVIEWED_NEW_PRODUCTS_V8_BOOTSTRAP_SHA256 =
  TEN_REPS_REVIEWED_NEW_PRODUCTS_V8.bootstrap_profile.sha256;
  ...
  [SIX_PACK_REVIEWED_FAMILY_BATCH, SIX_PACK_REVIEWED_MISSING_VARIANTS_BATCH, SIX_PACK_REVIEWED_FAMILY_MAP_BATCH, SIX_PACK_REVIEWED_FAMILY_MAP_FINAL, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V8, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V9, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V10, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V11, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V12, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V13, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V14, SIX_PACK_REVIEWED_LARGE_FAMILY_BATCH_V15]
    .flatMap((batch) => (batch.rows || batch.families.flatMap((family) =>
      family.variants.map((variant) => ({
        ...variant,
        product_id: family.product_id || null,
        approved_product_name: family.name || null,
...
function applyReviewedCanonicalFeedCorrections(row, options = {}) {
  ...
  if (isTenRepsReviewedSource) {
    ...
    const exactTextFields = [
      ...
      ["external_gtin", ""],
      ["product_name", tenRepsReviewedRow.product_name],
      ["slug", tenRepsReviewedRow.slug],
      ["brand", tenRepsReviewedRow.brand],
      ["category", tenRepsReviewedRow.category],
      ["description", ""],
      ["product_format", tenRepsReviewedRow.product_format],
      ["image", tenRepsReviewedRow.image],
      ["external_url", tenRepsReviewedRow.source_url],
      ["affiliate_url", tenRepsReviewedRow.source_url],
      ["pack_count", "1"],
      ["variant_name", tenRepsReviewedRow.variant_name],
      ["flavour", tenRepsReviewedRow.flavour || ""],
      ["size", `${tenRepsReviewedRow.size} ${tenRepsReviewedRow.size_unit}`],
      ["size_unit", tenRepsReviewedRow.size_unit],
    ];
    ...
    if (
      String(row.total_price ?? "").trim() &&
      normalizeDecimalString(row.total_price, "total_price") !==
        addDecimalStrings(expectedPrice, "3.99")
    ) {
      throw new Error("10 Reps reviewed new-product delivered price mismatch");
    }
    row = {
      ...row,
      __reviewed_10reps_new_product_identity: {
        ...
        external_variant_id: tenRepsReviewedRow.external_variant_id,
        flavour: tenRepsReviewedRow.flavour,
        size_value: String(tenRepsReviewedRow.size),
        size_unit: tenRepsReviewedRow.size_unit,
        product_format: tenRepsReviewedRow.product_format,
        source_url: tenRepsReviewedRow.source_url,
      },
    };
  }
  ...
  const isPredatorsPostCreateSiblingSource =
    predatorsSourceSha === PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING_SHA256;
  ...
  if (
    ...
    !isPredatorsPostCreateSiblingSource
  ) {
  ...
  if (
    ...
    if (
      isPredatorsPostCreateSiblingSource &&
      !PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING.included_review_rows.includes(
        predatorsReviewedRow.review_row
      )
    ) {
      throw new Error("Predators Gear sibling row is outside the approved remaining scope");
    }
    const exactTextFields = [
      ["retailer_name", PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.retailer.name],
      ["retailer_website", PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.retailer.website],
      ["external_product_id", predatorsReviewedRow.external_product_id],
      ["external_variant_id", predatorsReviewedRow.external_variant_id],
      ["external_sku", predatorsReviewedRow.external_sku],
      ["external_gtin", predatorsReviewedRow.external_gtin],
      ["product_name", predatorsReviewedRow.product_name],
      ["slug", predatorsReviewedRow.slug],
      ["brand", predatorsReviewedRow.brand],
      ["category", predatorsReviewedRow.category],
      ["description", ""],
      ["product_format", predatorsReviewedRow.product_format],
      ["image", predatorsReviewedRow.image],
      ["external_url", predatorsReviewedRow.source_url],
      ["affiliate_url", predatorsReviewedRow.source_url],
      ["pack_count", "1"],
    ];
    for (const [field, expected] of exactTextFields) {
      if (String(row[field] ?? "").trim() !== String(expected ?? "")) {
    ...
    const reviewedVariant =
      predatorsReviewedRow.action === "create_reviewed_product_variant";
    const expectedOptions = reviewedVariant
      ? canonicalJson(predatorsReviewedRow.external_options)
      : null;
    const actualOptions = parseExternalOptions(row.external_options);
    if (
      (reviewedVariant && canonicalJson(actualOptions) !== expectedOptions) ||
      (!reviewedVariant && actualOptions !== null)
    ) {
      throw new Error("Predators Gear reviewed new-product external_options mismatch");
    }
    const conditionalFields = reviewedVariant
      ? [
          ["variant_name", predatorsReviewedRow.variant_name],
          ["flavour", predatorsReviewedRow.flavour],
          ["size", `${predatorsReviewedRow.size} ${predatorsReviewedRow.size_unit}`],
          ["size_unit", predatorsReviewedRow.size_unit],
        ]
      : [
          ["variant_name", ""],
          ["flavour", ""],
    ...
    if (isPredatorsPostCreateReviewedSource || isPredatorsPostCreateSiblingSource) {
    ...
    if (
      normalizeDecimalString(row.price, "price") !==
        normalizeDecimalString(predatorsReviewedRow.price, "price") ||
      ...
      throw new Error("Predators Gear reviewed new-product commercial fields mismatch");
    }
    row = {
      ...row,
      __reviewed_predators_new_product_identity: {
        contract: PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.kind,
        review_row: predatorsReviewedRow.review_row,
        action: predatorsReviewedRow.action,
        external_product_id: predatorsReviewedRow.external_product_id,
        external_variant_id: predatorsReviewedRow.external_variant_id,
        flavour: predatorsReviewedRow.flavour || null,
        size_value: predatorsReviewedRow.size || null,
        size_unit: predatorsReviewedRow.size_unit || null,
        product_format: predatorsReviewedRow.product_format,
        source_url: predatorsReviewedRow.source_url,
        post_create_sibling: isPredatorsPostCreateSiblingSource,
      },
    };
  }
  ...
  const largeNewProductIdentity =
    ...
    productName === sixPackReviewed.approved_product_name &&
    String(row.slug || "").trim() ===
      sixPackReviewed.approved_product_slug &&
    String(row.brand || "").trim() === sixPackReviewed.approved_brand &&
    String(row.category || "").trim() ===
      sixPackReviewed.approved_category;
  const approvedProductIdentity =
    largeNewProductIdentity ||
    optionalIdentifier(row.product_id) ===
      String(sixPackReviewed?.product_id);
  const approvedVariantIdentity = isLargeSixPackBatch
    ? Boolean(optionalIdentifier(row.product_variant_id)) &&
      (!sixPackReviewed?.product_variant_id ||
        optionalIdentifier(row.product_variant_id) ===
          String(sixPackReviewed.product_variant_id))
    : [
  ...
  if (exactSixPackReviewedIdentity) {
    row = {
      ...
      __reviewed_six_pack_family_identity: {
        contract: sixPackReviewedBatch.kind,
        external_variant_id: externalVariantId,
        canonical_product_id: optionalIdentifier(row.product_id),
...
function normalizeCanonicalRetailerFeedRows(rows, options = {}) {
  ...
  if (tenRepsV8Profile) {
    ...
    if (
      ...
      throw new Error("10 Reps reviewed new-product bootstrap row set mismatch");
    }
  }
  if (
    String(options.sourceFileSha256 || "").toLowerCase() ===
    PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_SHA256
  ) {
    if (rows.length !== PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.canonical_csv.row_count) {
      throw new Error("Predators Gear reviewed new-product contract requires exactly 5 rows");
    }
    const actualVariantIds = rows
      .map((row) => optionalIdentifier(row.external_variant_id))
      .sort();
    const reviewedVariantIds = [
      ...PREDATORS_GEAR_REVIEWED_NEW_PRODUCT_ROWS.keys(),
    ].sort();
    if (
      new Set(actualVariantIds).size !== reviewedVariantIds.length ||
      canonicalJson(actualVariantIds) !== canonicalJson(reviewedVariantIds)
  ...
  if (
    String(options.sourceFileSha256 || "").toLowerCase() ===
    PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING_SHA256
  ) {
    if (rows.length !== PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING.row_count) {
      throw new Error("Predators Gear sibling contract requires exactly 2 rows");
    }
    const actualVariantIds = rows
      .map((row) => optionalIdentifier(row.external_variant_id))
      .sort();
    const reviewedVariantIds = PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_POST_CREATE_SIBLING
      .included_review_rows
      .map((reviewRow) => PREDATORS_GEAR_REVIEWED_NEW_PRODUCTS_V1.rows
        .find((row) => row.review_row === reviewRow).external_variant_id)
      .sort();
...
function collectCanonicalVariantEvidence(row) {
  ...
  if (
    ...
    const reviewedSize = parseSize(
      `${reviewedPredators.size_value} ${reviewedPredators.size_unit}`
    );
    const reviewedFlavour = normalizeFlavour(reviewedPredators.flavour);
    return {
      flavour: reviewedFlavour,
      size: reviewedSize,
      packCount: parsePackCount(
        row.pack_count ? `pack of ${row.pack_count}` : ""
      ),
      productFormat: parseProductFormat(reviewedPredators.product_format),
      discriminatingSupplied: true,
      supplied: true,
    };
  }
...
async function validateLegacyMappingUpgrade({
  ...
  const reviewedGymHighNoSkuIdentity =
    (controls.optioned || controls.standalone) &&
...
const REVIEWED_PARENT_VARIANT_POLICY = new Map([
  ...
  ["Time 4 Glutamine 600g 120 Servings", { brand: "Time 4 Nutrition", category: "Amino Acids", format: "powder", size: "600:g", tenRepsV8Only: true, allowUnflavoured: true, simpleProduct: true }],
...
function assertReviewedParentVariantPolicy(row, rowNumber, evidence) {
  ...
  const externalVariantId = optionalIdentifier(row.external_variant_id);
  const externalOptions = parseExternalOptions(row.external_options);
  const retailerSlug = slugifyRetailerName(
    required(row.retailer_name, "retailer_name", rowNumber)
  );
  const reviewedTenReps = row.__reviewed_10reps_new_product_identity;
  const reviewedPredators = row.__reviewed_predators_new_product_identity;
  if (reviewedTenReps) {
    ...
    if (
      ...
      throw new Error("reviewed 10 Reps parent requires numeric product and variant IDs");
    }
    if (
      policy.simpleProduct
        ? externalProductId !== externalVariantId
        : externalProductId === externalVariantId
    ) {
      throw new Error("reviewed 10 Reps parent product/variant identity mismatch");
    }
    const directUrl = getDirectRetailerProductUrl(row) || getRetailerProductUrl(row);
    if (
      directUrl !== reviewedTenReps.source_url ||
      !isLikelyWooCommerceProductUrl(row, externalProductId)
    ) {
      throw new Error("reviewed 10 Reps parent URL mismatch");
    }
  } else if (reviewedPredators) {
    ...
    if (
      ...
      throw new Error("reviewed Predators Gear parent requires distinct numeric product and variant IDs");
    }
    const directUrl = getDirectRetailerProductUrl(row) || getRetailerProductUrl(row);
    if (directUrl !== reviewedPredators.source_url) {
  ...
  if (
    ...
    throw new Error(
      reviewedPredators || reviewedTenReps
        ? "reviewed parent explicit-variant requires exact WooCommerce flavour option"
        : "reviewed parent explicit-variant requires exact Shopify flavour option"
    );
  }
  ...
  if (optionSizes.length > 0 && (
    ...
    throw new Error(
      reviewedPredators || reviewedTenReps
        ? "reviewed parent explicit-variant WooCommerce size option mismatch"
        : "reviewed parent explicit-variant Shopify size option mismatch"
    );
  }
...
async function resolveFeedRow(row, rowNumber, options = {}) {
  ...
  if (
    ...
    shippingNormalizedRow.__reviewed_predators_new_product_identity?.post_create_sibling
  ) {
    try {
  ...
  if (safeCreate && validationErrors.length === 0) {
    ...
    if (!product) {
      ...
      if (
        ...
        !isSafeCreateRowAmbiguous(shippingNormalizedRow)
      ) {
