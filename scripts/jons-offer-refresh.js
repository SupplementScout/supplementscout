...
async function buildRun(target,state,diagnostic=null,reviewed=null,isolateUnsafe=false){
  ...
  const missingReconciliation=reconcileReviewedMissingVariants(state.records,sourceVariants);
  ...
  if(reviewed){
    ...
    if(aggregateReasons.has(classification.reason)||hardPriceOfferIds.length){
      ...
      const confirmation=await confirmAggregateSource(state,firstMappedFingerprint,diagnostic);
  ...
  return{target,spec,capturedAt,snapshot,sourceVariants,classification,artifacts,manifest,manifestFingerprint:canonicalHash(manifest),binding,head,discovery,reviewed,reviewedExpiresAt,reviewedContract,scopedSourceEvidence,mappedSourceEvidence,isolateUnsafe,automaticPriceConfirmation};
}
...
async function executeRefresh(args,diagnostic){
  ...
  const reviewRows=quarantined.map(row=>({offer_id:row.offer_id,retailer_product_id:row.retailer_product_id,external_product_id:row.external_product_id,external_variant_id:row.external_variant_id,reason:row.reason,old_price:row.target.price,new_price:row.source.price,old_stock:row.target.in_stock,new_stock:row.source.in_stock}));
