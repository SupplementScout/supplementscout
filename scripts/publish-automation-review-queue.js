...
function parseArgs(argv) { const out = {}; for (const arg of argv) { const match = arg.match(/^--(input|mode|confirm)=(.+)$/); if (!match || out[match[1]]) fail(`Invalid argument ${arg}`); out[match[1]] = match[2]; } if (!out.input || !["dry-run", "apply"].includes(out.mode)) fail("Required --input and --mode=dry-run|apply"); if (out.mode === "apply" && out.confirm !== "OWNER_APPROVED_REVIEW_QUEUE_EXACT_375") fail("Exact review-only confirmation required"); return { ...out, input: path.resolve(out.input) }; }
