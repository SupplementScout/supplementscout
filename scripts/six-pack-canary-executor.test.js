...
test("executor accepts the exact reviewed V13 16-offer accessory rollout", () => {
  const csv = path.join(
    ROOT,
    "config",
    "retailers",
    "six-pack-production-expansion-v13.csv"
  );
  const rolloutPath = path.join(
    ROOT,
    "config",
    "retailers",
    "six-pack-production-expansion-v13.json"
  );
  const rollout = JSON.parse(fs.readFileSync(rolloutPath, "utf8"));
  const loaded = {
    artifact: {
...
test("executor accepts the exact reviewed V14 49-offer rollout", () => {
  const csv = path.join(
    ROOT,
    "config",
    "retailers",
    "six-pack-production-expansion-v14.csv"
  );
  const rolloutPath = path.join(
    ROOT,
    "config",
    "retailers",
    "six-pack-production-expansion-v14.json"
  );
  const rollout = JSON.parse(fs.readFileSync(rolloutPath, "utf8"));
  const loaded = {
    artifact: {
...
test("executor accepts the exact reviewed V15 66-offer rollout", () => {
  const csv = path.join(
    ROOT,
    "config",
    "retailers",
