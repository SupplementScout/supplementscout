...
create or replace function public.register_fit_house_offer_sync_control_plan(
  p_request jsonb
) returns jsonb
language plpgsql
volatile
...
begin
  ...
  if p_request->>'retailer_id' <> '9'
     or p_request->>'retailer_slug' <> 'fit-house'
     or p_request->>'source_platform' <> 'SHOPIFY'
     or lower(p_request->>'source_domain') <> 'fithouse.uk'
     ...
     or not exists(
       ...
       where id=9 and slug='fit-house' and website='https://fithouse.uk'
     ) then
    perform public.retailer_catalogue_raise(
      'RSBI_ENVIRONMENT_BLOCKED',
      'Registration is restricted to the frozen Fit House Shopify manifest'
    );
  end if;
  ...
  if jsonb_typeof(v_manifest) is distinct from 'array'
     or jsonb_array_length(v_manifest) <> 286 then
    perform public.retailer_catalogue_raise(
      'RSBI_EXPECTED_STATE_MISMATCH',
      'Manifest must contain exactly 286 approved Fit House mappings'
    );
  end if;
  ...
  for v_entry in select value from jsonb_array_elements(v_manifest) loop
    ...
    if v_mapping_id <= v_previous_mapping_id
      ...
      perform public.retailer_catalogue_raise(
        ...
        'Manifest mappings must be unique, ascending '
      );
    end if;
    ...
    if v_mapping.id is null or v_offer.id is null
       or v_mapping.retailer_id <> 9 or v_offer.retailer_id <> 9
      ...
      perform public.retailer_catalogue_raise(
        'RSBI_EXPECTED_STATE_MISMATCH',
        'Manifest row is not an approved Fit House mapping/offer identity'
      );
    end if;
  ...
  if exists(
    ...
    perform public.retailer_catalogue_raise(
      'RSBI_EXPECTED_STATE_MISMATCH',
      'Manifest omits an approved exact Fit House mapping or offer'
    );
  end if;
  ...
  for v_child in select value from jsonb_array_elements(v_children) loop
    ...
    if v_artifact->>'retailer_id' <> '9'
       or v_artifact->>'retailer_slug' <> 'fit-house'
    ...
    for v_row in select value from jsonb_array_elements(v_rows) loop
      ...
      if not exists(
        ...
        perform public.retailer_catalogue_raise(
          'RSBI_EXPECTED_STATE_MISMATCH',
          'Child contains an identity outside the approved Fit House manifest'
        );
      end if;
    ...
    v_child_manifest := v_child_manifest||jsonb_build_array(
      jsonb_build_object(
        ...
        'dependency_group','retailer-offer-sync:9:'||v_source_fingerprint,
        'rollback_group','retailer-offer-sync:9:'||v_source_fingerprint,
  ...
  v_parent_hash_input := jsonb_build_object(
    ...
    'retailer_id','9','source_country','GB',
  ...
  perform pg_advisory_xact_lock(hashtextextended(v_target||':9',0));
  if exists(
  ...
  if exists(
    select 1 from public.retailer_catalogue_parent_plans
    where retailer_id=9 and target_environment=v_target
      and status in ('PLANNED','APPROVED','PARTIALLY_APPLIED','FAILED')
  ) then
    perform public.retailer_catalogue_raise(
      'RSBI_REPLAY_BLOCKED',
      'An equivalent Fit House control plan is already active'
    );
  end if;
  ...
  insert into public.retailer_catalogue_parent_plans(
    ...
    v_parent_id,v_parent_fingerprint,9,v_target,v_source_fingerprint,
  ...
  for v_child in select value from jsonb_array_elements(v_children) loop
    ...
    insert into public.retailer_catalogue_child_plans(
      ...
      (v_child->>'child_plan_id')::uuid,v_parent_id,9,v_target,
...
$register$;
alter function public.register_fit_house_offer_sync_control_plan(jsonb)
  owner to postgres;
revoke all on function public.register_fit_house_offer_sync_control_plan(jsonb)
  from public,anon,authenticated,service_role;
do $grants$
begin
  if exists(select 1 from pg_roles where rolname='retailer_catalogue_staging_validator') then
    grant execute on function public.register_fit_house_offer_sync_control_plan(jsonb)
