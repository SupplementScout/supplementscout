...
- The required active-function test stopped before approval or apply with `function digest(text, unknown) does not exist`. Production has `extensions.digest(text,text)`, while `validate_reviewed_variant_create_rebind_offer_update_plan(jsonb)` is intentionally pinned to `pg_catalog, public, pg_temp` and contained four unqualified calls. No catalogue, approval-ledger, execution-ledger or Review Queue write occurred.
- Forward-only migration `20260901100000_fix_reviewed_variant_digest_schema_resolution.sql`, SHA-256 `aaf408391412c3786a2b860b00989e0bad78ab511cbde57b8719a8656a6eea49`, requires the exact production target and active definition shape, obtains its exact `CREATE OR REPLACE FUNCTION` definition, changes only the four calls to `extensions.digest(...)`, and revalidates owner, `SECURITY DEFINER`, pinned search path and ACL. It contains no DML or grants.
- Staging apply is not applicable: both the base reviewed operation and this repair are production-only exclusions, while staging has an unrelated pending Review Queue migration. No unrelated staging migration was rehearsed or applied. The isolated PostgreSQL contract instead installs `pgcrypto` in `extensions`, reproduces the production resolution failure, proves fingerprint calculation after repair, contract-level invalid-request and stale-state failures, zero fail-closed deltas, final-step rollback, optioned legacy dispatch, ACL denial and idempotent replay.
- Targeted tests, migration/selector tests, `verify:quick`, `verify:full`, baseline validation, production Next.js build and Project Guardian passed. The repair is not deployed. No approval RPC, offer apply, replay, cron or catalogue write was performed. Next gate is explicit owner approval for this one repair migration only; only after active production postflight passes may a new two-capture offer artifact be created.
