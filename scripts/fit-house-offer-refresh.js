...
function selectOwnerApprovedSixExecutionRows(classification,ownerApprovedSix){
  ...
  const batchCount=Math.ceil(selected.length/3),confirmationCount=17*batchCount;
...
async function executeRefresh(args,diagnostic,reviewed=null){
  ...
  if(run.classification.action==="OWNER_APPROVED_SIX_STOCK_ONLY")invariant(stockChangeOfferIds.length+verificationOfferIds.length===executionOfferIds.length&&stockChangeOfferIds.length<=6&&verificationOfferIds.length===17*Math.ceil(stockChangeOfferIds.length/3),"Fit House six-offer report scope mismatch");
