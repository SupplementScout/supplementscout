...
const KIND = "ebay-offer-batch-h-exact-11-v1";
const CONFIRMATION = "OWNER_APPROVED_EBAY_BATCH_H_EXACT_11";
const PROJECT_REF = "aftboxmrdgyhizicfsfu";
const SELLER = "appliednutritionplc";
const ROLLOUT_FINGERPRINT = "1d88e534d0678be4e0c1238027c1cb0c0a012947cccd1f6d7bcc54ac8dc1bbb2";
const ROLLOUT_PATH = path.join(ROOT, "docs", "rollouts", "ebay-offer-canary", "batch-h-rollout.json");
...
function validateRollout() {
  ...
  if (rollout.schema_version !== 1 || rollout.kind !== KIND || rollout.approved !== true || rollout.owner_confirmation !== CONFIRMATION || rollout.target_project_ref !== PROJECT_REF || rollout.seller !== SELLER || rollout.rollout_fingerprint !== ROLLOUT_FINGERPRINT || fingerprint !== ROLLOUT_FINGERPRINT || JSON.stringify(identities) !== JSON.stringify(EXPECTED_IDENTITIES)) fail("Batch H rollout approval, target, scope or fingerprint mismatch");
...
async function validateLiveSources(fetchImpl = fetch, env = process.env) {
  ...
  for (const { approved } of entries) {
    ...
    if (!response.ok) fail(`Batch H item ${approved.external_variant_id} direct read failed with HTTP ${response.status}`);
    ...
    const returnedGtin = String(item.gtin || aspect(item, "EAN") || "");
    const expectedFlavour = approved.flavour === "strawberries and cream" ? "strawberry and cream" : approved.flavour;
    if (String(item.itemId) !== approved.external_variant_id || String(item.legacyItemId) !== approved.external_product_id || returnedGtin !== approved.gtin || normalized(aspect(item, "Flavour")) !== normalized(expectedFlavour) || item.listingMarketplaceId !== "EBAY_GB" || !item.buyingOptions?.includes("FIXED_PRICE") || (item.conditionId !== "1000" && normalized(item.condition) !== "new") || !inStock || item.seller?.username !== SELLER || item.seller?.sellerAccountType !== "BUSINESS" || item.seller?.sellerLegalInfo?.name !== "Applied Nutrition PLC" || Number(item.seller?.feedbackPercentage) < 98 || Number(item.seller?.feedbackScore) < 100 || String(itemPrice) !== approved.price || String(shipping) !== approved.shipping_cost || String(delivered) !== approved.total_price || !String(item.itemAffiliateWebUrl || "").includes(`campid=${config.campaign_id}`) || (Number(approved.pack_count) === 8 && !/8\s*x\s*500ml/i.test(item.title)) || /\b(sample|sachet|bundle|damaged|opened|used|refurbished|empty container)\b/i.test(item.title)) fail(`Batch H live safety evidence drift for ${approved.external_variant_id}`);
    rows.push({ product_id: approved.product_id, product_variant_id: approved.product_variant_id, item_id: approved.external_variant_id, seller: SELLER, gtin: returnedGtin, delivered_price: delivered });
  }
  return rows;
}

async function run(options, dependencies = {}) {
  if (process.env.GITHUB_ACTIONS !== "true" || process.env.GITHUB_REF !== "refs/heads/main" || process.env.GITHUB_EVENT_NAME !== "workflow_dispatch" || process.env.EBAY_CANARY_OWNER_CONFIRMATION !== CONFIRMATION) fail("Batch H production apply requires exact owner-approved GitHub Actions dispatch on main");
