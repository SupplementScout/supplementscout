...
test("RA-004 migration, roles, ledger, eleven sources and one snapshot pass on isolated local PostgreSQL", async () => {
  ...
  try {
    ...
    const rcSession="snapshot-session-read-committed",rcLock="snapshot-lock-read-committed";
    const readCommitted=asyncSql(container,database,`begin; select 'isolation|'||current_setting('transaction_isolation'); select 'rc_before|'||(public.read_retailer_control_state_v1(14,'10 Reps','${"a".repeat(40)}','${"b".repeat(64)}',statement_timestamp()+interval '10 minutes',array[${SOURCE_NAMES.map(q).join(",")}],10000,8388608)->>'canonical_state_fingerprint'); select pg_sleep(2); select 'rc_after|'||(public.read_retailer_control_state_v1(14,'10 Reps','${"a".repeat(40)}','${"b".repeat(64)}',statement_timestamp()+interval '10 minutes',array[${SOURCE_NAMES.map(q).join(",")}],10000,8388608)->>'canonical_state_fingerprint'); commit;`);
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,500);
    ok(sql(container,database,`begin; update public.retailer_catalogue_parent_plans set status='EXPIRED' where id='10000000-0000-4000-8000-000000000001'; update public.retailer_catalogue_production_fixture_approvals set consumed_at=now() where id='80000000-0000-4000-8000-000000000001'; ${eventSql({event_type:"SESSION_STARTED",status:"ACTIVE",source_run_id:rcSession})} ${eventSql({event_type:"LOCK_ACQUIRED",status:"ACTIVE",source_run_id:rcLock,metadata:{owner_session_id:rcSession}})} ${eventSql({event_type:"WORKFLOW_OBSERVED",status:"OBSERVED",source_run_id:"snapshot-evidence-read-committed"})} commit;`),"read committed atomic state transition");
    const readCommittedResult=await readCommitted; assert.equal(readCommittedResult.status,0,output(readCommittedResult));
    const readCommittedLines=readCommittedResult.stdout.split(/\r?\n/).filter((line)=>/^(isolation|rc_before|rc_after)\|/.test(line)); assert.equal(readCommittedLines.length,3); assert.equal(readCommittedLines[0],"isolation|read committed"); assert.equal(readCommittedLines[1].split("|")[1],after.canonical_state_fingerprint);
    const readCommittedAfter=jsonResult(ok(sql(container,database,readSql()),"read committed post-transition snapshot")); assert.notEqual(readCommittedAfter.canonical_state_fingerprint,after.canonical_state_fingerprint); assert.equal(readCommittedLines[2].split("|")[1],readCommittedAfter.canonical_state_fingerprint); assert.ok(readCommittedAfter.source_records.sessions.some((row)=>row.id===rcSession&&row.status==="OPEN")); assert.ok(readCommittedAfter.source_records.locks.some((row)=>row.id===rcLock&&row.status==="ACTIVE")); assert.ok(readCommittedAfter.source_records.approval_consumption.some((row)=>row.approval_id==="80000000-0000-4000-8000-000000000001"));

    const hijack=jsonResult(ok(sql(container,database,`set session authorization retailer_control_state_exporter; create function pg_temp.sha256(bytea) returns bytea language sql immutable as 'select decode(repeat(''00'',32),''hex'')'; ${readSql()}`),"pg_temp hijack resistance")); assert.equal(hijack.canonical_state_fingerprint,readCommittedAfter.canonical_state_fingerprint);
