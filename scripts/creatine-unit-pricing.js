...
const PLAN_KIND = "creatine-unit-pricing-verification-plan-v1";
const AUDIT_KIND = "creatine-unit-pricing-verification-audit-v1";
const PRODUCTION = CONTRACTS.PRODUCTION;
const SELECT_FIELDS = [
  "id", "name", "category", "product_format", "net_weight_g", "serving_count_verified",
  "serving_size_g", "creatine_per_serving_g", "unit_count", "unit_type", "nutrition_verified", "unit_pricing_verified",
...
function parseArgs(argv) {
  ...
  for (const argument of argv) {
    ...
    else if (argument.startsWith("--product-ids=")) {
      const values = argument.slice("--product-ids=".length).split(",").map((value) => value.trim());
  ...
  if (options.mode === "apply" && options.productIds) fail("Apply uses the product IDs sealed inside the plan");
  return options;
}
...
function snapshot(product) {
  return {
    ...
    creatine_per_serving_g: positive(product.creatine_per_serving_g),
    unit_count: positive(product.unit_count),
    unit_type: product.unit_type == null ? null : String(product.unit_type).trim().toLowerCase(),
...
function classifyProduct(product) {
  ...
  if (value.creatine_per_serving_g === null) reasons.push("CREATINE_MISSING");
  if (value.product_format === "powder") {
    if (value.net_weight_g === null) reasons.push("NET_WEIGHT_MISSING");
    if (value.serving_size_g === null) reasons.push("SERVING_SIZE_MISSING");
    if (value.creatine_per_serving_g !== null && value.serving_size_g !== null &&
        value.creatine_per_serving_g > value.serving_size_g) reasons.push("CREATINE_EXCEEDS_SERVING");
    if (value.net_weight_g !== null && value.serving_count_verified !== null && value.serving_size_g !== null) {
      ...
      if (value.serving_count_verified * value.serving_size_g > value.net_weight_g + tolerance) reasons.push("PACKAGE_SERVING_MISMATCH");
    }
  } else if (["capsule", "tablet", "gummy"].includes(value.product_format)) {
    if (value.unit_count === null || !Number.isInteger(value.unit_count)) reasons.push("UNIT_COUNT_INVALID");
    if (value.unit_type !== value.product_format) reasons.push("UNIT_TYPE_FORMAT_MISMATCH");
    if (value.unit_count !== null && value.serving_count_verified !== null && value.serving_count_verified > value.unit_count) reasons.push("SERVINGS_EXCEED_UNIT_COUNT");
  } else {
    reasons.push("UNSUPPORTED_PRODUCT_FORMAT");
  }
  return { eligible: reasons.length === 0, reasons, snapshot: value };
}

function buildPlan(products, generatedAt = new Date().toISOString(), requestedProductIds = null) {
  const requested = requestedProductIds ? new Set(requestedProductIds.map(String)) : null;
  const updates = products.filter((product) => !requested || requested.has(String(product.id)))
    ...
    .map((entry) => ({
      ...
      before_snapshot_fingerprint: fingerprint("CREATINE_UNIT_PRICING_BEFORE", entry.result.snapshot),
  ...
  const core = {
    ...
    eligibility: {
      creatine_scope_required: true,
      nutrition_verified_required: true,
      product_format_required: "powder or verified counted format",
      complete_package_or_unit_arithmetic_required: true,
      requested_product_ids: requested ? [...requested].sort((left, right) => Number(left) - Number(right)) : null,
    },
    product_updates: updates,
    database_writes: 0,
  };
  return { ...core, plan_fingerprint: fingerprint("CREATINE_UNIT_PRICING_PLAN", core) };
}
...
function validatePlan(plan) {
  ...
  if (plan.plan_fingerprint !== fingerprint("CREATINE_UNIT_PRICING_PLAN", core)) fail("Unit-pricing plan fingerprint mismatch");
  const ids = new Set();
  for (const update of plan.product_updates) {
    if (!/^[1-9]\d*$/.test(String(update.product_id)) || ids.has(String(update.product_id)) ||
        ...
        update.before_snapshot_fingerprint !== fingerprint("CREATINE_UNIT_PRICING_BEFORE", update.before_snapshot) ||
...
function tmpPlanPath(plan, cwd = process.cwd()) {
  const directory = path.resolve(cwd, "tmp", "creatine-unit-pricing");
...
async function applyPlan(plan, dependencies = {}) {
  ...
  const client = dependencies.client || new Client({
    ...
    application_name: "supplementscout-creatine-unit-pricing",
  });
  ...
  try {
    ...
    await client.query("select pg_advisory_xact_lock(hashtextextended('supplementscout:creatine-unit-pricing',0))");
    ...
    for (const update of plan.product_updates) {
      ...
      if (fingerprint("CREATINE_UNIT_PRICING_BEFORE", current) !== update.before_snapshot_fingerprint || !classifyProduct(current).eligible) {
        fail(`Product ${update.product_id} changed after plan generation`);
      }
    }
    for (const update of plan.product_updates) {
      const changed = await client.query("update public.products set unit_pricing_verified=true where id=$1 and unit_pricing_verified is not true returning id", [update.product_id]);
...
async function runCli(argv = process.argv.slice(2), dependencies = {}) {
  ...
  if (options.mode === "plan") {
    ...
    const plan = buildPlan(products, dependencies.generatedAt, options.productIds);
    if (options.productIds && plan.product_updates.length !== options.productIds.length) {
      fail("One or more requested products failed creatine unit-pricing eligibility");
    }
