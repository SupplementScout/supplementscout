...
- Routine automation is implemented in `.github/workflows/six-pack-offer-refresh.yml`. One shared expandable manifest drives the complete retailer scope; the workflow runs daily at `03:17 UTC` and supports manual dry-run/apply. Every run tests contracts, fetches five exact WooCommerce product pages for the six approved variants, validates source identity and full manifest coverage, applies only approved existing mappings through separate production approver/executor roles, takes a fresh source capture, requires idempotency and uploads evidence.
- Automatic apply fails closed on a missing/duplicate source identity, stale source, URL/domain drift, commercial identity drift, unexpected mapping/offer, catalogue create, hard per-row price anomaly, two or more changed rows, two or more price changes, two or more new out-of-stock transitions, excessive total OOS or post-apply drift. New products are never created by this workflow.
...
- Status: **PRODUCTION CANARY COMPLETE / SHARED DAILY AUTOMATION OPERATIONAL**.
