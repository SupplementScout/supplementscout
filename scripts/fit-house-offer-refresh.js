...
const OUT=path.join(ROOT,"tmp","fit-house-offer-refresh");
...
function loadApprovedManifest(){
  ...
  invariant(manifest.retailer?.id===9&&manifest.retailer?.slug==="fit-house"&&manifest.approved_mapping_count===286&&manifest.rows?.length===286,"approved manifest shape mismatch");
  invariant(new Set(manifest.rows.map(row=>String(row.external_variant_id))).size===286,"approved manifest duplicate variant identity");
  return{manifest,sha256:actual};
}
function sourceHealth(snapshot,sourceVariants){
  const baseline=config.source_baseline;
  invariant(baseline&&baseline.product_count>0&&baseline.variant_count>0,"Fit House source baseline missing");
...
function diagnosticName(argv,env=process.env){
  ...
  const phase=String(env.FIT_HOUSE_REFRESH_PHASE||mode).replace(/[^a-z0-9_-]+/gi,"-").toLowerCase();
...
function diagnosticTemplate(argv,env=process.env){
  ...
  return{
    ...
    trigger_type:env.FIT_HOUSE_REFRESH_TRIGGER_TYPE||env.GITHUB_EVENT_NAME||"local",
    commit,
...
function balancedExecutionBatches(rows,maximumBatchSize=50){invariant(Number.isInteger(maximumBatchSize)&&maximumBatchSize>0,"invalid maximum batch size");if(rows.length===0)return[];const count=Math.ceil(rows.length/maximumBatchSize),batches=Array.from({length:count},()=>[]),newOos=[],existingOos=[],inStock=[];for(const row of rows){const before=Boolean(row.atomic_plan.expected_state.offer.in_stock),after=Boolean(row.atomic_plan.offer.values.in_stock);if(!after&&before)newOos.push(row);else if(!after)existingOos.push(row);else inStock.push(row)}let cursor=0;for(const group of[newOos,existingOos,inStock])for(const row of group){while(batches[cursor%count].length>=maximumBatchSize)cursor+=1;batches[cursor%count].push(row);cursor+=1}for(const batch of batches){batch.sort((a,b)=>Number(a.offer_id)-Number(b.offer_id));invariant(batch.length>0&&batch.length<=maximumBatchSize,"balanced batch size drift")}invariant(batches.reduce((sum,batch)=>sum+batch.length,0)===rows.length,"balanced batch coverage drift");return batches}
...
async function readState(target){
  ...
  if(url&&key&&new URL(url).hostname.split(".")[0]===spec.ref){const client=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});[retailers,products,variants,mappings,offers,history]=await Promise.all([
      all(client,"retailers","id,name,slug,website",q=>q.eq("id",9)),all(client,"products","id,name,is_active,merged_into_product_id,product_format"),all(client,"product_variants","id,product_id,variant_key,display_name,flavour_code,flavour_label,size_value,size_unit,pack_count,product_format,is_active,is_default"),all(client,"retailer_products","id,retailer_id,product_id,product_variant_id,external_product_id,external_variant_id,external_sku,external_options,external_name,external_slug,external_gtin,external_url,match_method,match_confidence,updated_at",q=>q.eq("retailer_id",9)),all(client,"offers","id,product_id,retailer_id,product_variant_id,retailer_product_id,price,shipping_cost,total_price,in_stock,url,last_checked_at",q=>q.eq("retailer_id",9)),all(client,"price_history","id")]);
  }else{invariant(target==="staging"&&!process.env.GITHUB_ACTIONS,"read-only Supabase target mismatch");const values=loadEnvFile(path.join(ROOT,".env.staging.audit.local")),connection=Object.entries(values).find(([name])=>name.endsWith("_DATABASE_URL"))?.[1];invariant(connection,"staging audit credential missing");const parsed=new URL(connection);parsed.searchParams.delete("sslmode");const client=new Client({connectionString:parsed.href,ssl:{rejectUnauthorized:false},application_name:"fit-house-offer-refresh-staging-read",options:"-c default_transaction_read_only=on -c statement_timeout=120000"});await client.connect();try{await client.query("begin read only");[retailers,products,variants,mappings,offers,history]=await Promise.all([
        client.query("select id,name,slug,website from public.retailers where id=9").then(r=>r.rows),client.query("select id,name,is_active,merged_into_product_id,product_format from public.products").then(r=>r.rows),client.query("select id,product_id,variant_key,display_name,flavour_code,flavour_label,size_value,size_unit,pack_count,product_format,is_active,is_default from public.product_variants").then(r=>r.rows),client.query("select id,retailer_id,product_id,product_variant_id,external_product_id,external_variant_id,external_sku,external_options,external_name,external_slug,external_gtin,external_url,match_method,match_confidence,to_char(updated_at at time zone 'UTC','YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') updated_at from public.retailer_products where retailer_id=9").then(r=>r.rows),client.query("select id,product_id,retailer_id,product_variant_id,retailer_product_id,price,shipping_cost,total_price,in_stock,url,to_char(last_checked_at at time zone 'UTC','YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') last_checked_at from public.offers where retailer_id=9").then(r=>r.rows),client.query("select id from public.price_history").then(r=>r.rows)]);await client.query("rollback")}finally{await client.end()}}
  invariant(retailers.length===1&&retailers[0].slug==="fit-house"&&retailers[0].website==="https://fithouse.uk","Fit House retailer mismatch");
  invariant(mappings.length===286&&offers.length===286,"Fit House approved scope must be 286/286");
  const approved=loadApprovedManifest(),expected=approved.manifest.rows.map(row=>`${row.external_product_id}:${row.external_variant_id}`).sort(),actual=mappings.map(row=>`${row.external_product_id}:${row.external_variant_id}`).sort();
  invariant(JSON.stringify(actual)===JSON.stringify(expected),"Fit House approved stable Shopify scope drift");
  ...
  invariant(new Set(records.map(row=>String(row.mapping.external_variant_id))).size===286,"duplicate approved external variant identity");
...
async function buildRun(target,state,diagnostic=null,reviewed=null){
  ...
  if(reviewed){
    ...
    if(classification.reason!=="MASS_OOS"
       || classification.rows.length!==286
      ...
      throw new RefreshError("REVIEWED_MANIFEST_DRIFT","live Fit House source/state differs from reviewed mixed-change manifest","REVIEWED_CONTRACT",{classifier_reason:classification.reason||null,reviewed_manifest_sha256:reviewed.sha256,reviewed_source_fingerprint:reviewed.manifest.source_capture_sha256,live_source_fingerprint:snapshot.semantic_source_fingerprint,reviewed_scope:expected,live_scope:stable});
    }
  }else if(classification.state!=="DRY_RUN_READY"||classification.rows.length!==286)throw new RefreshError(classification.reason||"CLASSIFIER_BLOCKED","full Fit House classifier blocked","CLASSIFIER",{...(classification.detail||{}),guard_evidence:classification.guard_evidence||null});
  ...
  return{target,spec,capturedAt,snapshot,sourceVariants,classification,artifacts,manifest,manifestFingerprint:canonicalHash({approved_manifest_sha256:config.manifest_sha256,environment:spec.environment,rows:manifest}),binding,head,discovery,reviewed,reviewedExpiresAt,reviewedContract,scopedSourceEvidence,mappedSourceEvidence};
}
...
module.exports={RefreshError,balancedExecutionBatches,buildRun,canonicalHash,classificationDiagnostic,diagnosticTemplate,executionRow,guardrailsFor,migrationBinding,parseArgs,readState,registrationRequest,runWithDiagnostic,sourceHealth,sumDeltas,verificationRecord};
