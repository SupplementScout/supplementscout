...
do $enable_gym_high_producer$
declare
  v_target jsonb := public.retailer_catalogue_actual_database_target();
  v_authority constant text := 'owner-chat-2026-08-26-unlock-gym-high-producer';
...
begin
  if current_user <> 'postgres'
     ...
     or v_authority <> 'owner-chat-2026-08-26-unlock-gym-high-producer' then
    raise exception 'GYM HIGH producer authority or production target mismatch';
  end if;
  ...
  if (select count(*) from public.price_observation_producers) <> 7
     or not exists(
       ...
       where retailer_id=10 and retailer_slug='jon-s-supplements'
         and source_importer='retailer_offer_mixed_batch'
         and approved_scope='reviewed-current-sync' and technically_capable and enabled
         and public_use='eligible-after-separate-approval'
         and terms_mode='standard-single-purchase-only'
     )
     or exists(
       select 1 from public.price_observation_producers
       where retailer_id<>10 and enabled
     ) then
    raise exception 'existing producer control state is not the approved Jon''s-only baseline';
  end if;
  ...
  if not exists(
    ...
    where retailer_id=1 and retailer_slug='gym-high'
      ...
      and approved_scope='reviewed-66' and technically_capable and not enabled
      and public_use='owner-deferred'
      and terms_mode='standard-single-purchase-only'
  ) or (select count(*) from public.retailer_products where retailer_id=1) <> 66
    or (select count(*) from public.offers where retailer_id=1) <> 66
    or (select count(*) from public.price_identity_series where retailer_id=1) <> 0 then
    raise exception 'GYM HIGH producer, reviewed scope, or pre-accrual state mismatch';
  end if;
  ...
  if (
    ...
    raise exception 'GYM HIGH exact-pack producer-ready scope is not the audited 40 of 66';
  end if;
  ...
  if v_rows<>1 then raise exception 'GYM HIGH producer enablement changed an unexpected row count'; end if;
  ...
  if (select count(*) from public.price_observation_producers where enabled)<>2
    ...
    raise exception 'GYM HIGH producer enablement postcondition failed';
  end if;
end
$enable_gym_high_producer$;
