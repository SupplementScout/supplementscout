...
const NUTRITION_PREWORKOUT_FACTS_SHA256 = "6f31c98f0af79c00214926026216e0e7b83485ee9e28ae7231ac904b2c5b5822";
...
const TEN_REPS_V8_SHORT_SOURCE_IDS_MIGRATION = "20260906150000_allow_10reps_v8_short_source_ids.sql";
const TEN_REPS_V8_SHORT_SOURCE_IDS_SHA256 = "f0b40f544c308164941987e94147d72409db141999a769805b803c4218d78b91";
const TEN_REPS_V8_SIBLING_VARIANTS_MIGRATION = "20260906153000_allow_10reps_v8_sibling_variants_without_default.sql";
const TEN_REPS_V8_SIBLING_VARIANTS_SHA256 = "372c8679c2fdab5cefea3427bb03494526071c7b4891830af3c5562be088119f";
const TEN_REPS_NEW_PRODUCTS_V9_MIGRATION = "20260906200000_allow_10reps_reviewed_new_products_v9.sql";
...
const TEN_REPS_V10_SHORT_SOURCE_MIGRATION = "20260907070000_allow_10reps_v10_short_source_ids.sql";
const TEN_REPS_V10_SHORT_SOURCE_SHA256 = "c966bee7f212c5acaff3eea9cb0c9d2781725dbcf5038eaeefe2e6dff08609ba";
...
const REVIEWED_EXISTING_CATEGORIES_MIGRATION = "20260908110000_allow_reviewed_catalogue_existing_categories.sql";
const REVIEWED_EXISTING_CATEGORIES_SHA256 = "07a1662c010f4e3c967f87aaa3709d1bb10f3bd139b5fa236653b40e271033cf";
const REVIEWED_VARIANT_COUNT_MIGRATION = "20260908113000_allow_reviewed_variant_count_evidence.sql";
...
test("staging records both verified no-change timestamp repairs as applied", () => {
  ...
  assert.deepEqual(result.pending_files, [REVIEW_QUEUE_PUBLICATION_MIGRATION, REVIEW_QUEUE_RETRY_MIGRATION, NUTRITION_VARIANT_PROVENANCE_MIGRATION, NUTRITION_PREWORKOUT_FACTS_MIGRATION]);
  ...
  assert.equal(result.selected_files.length, 98);
...
test("production records the nutrition migrations as pending", () => {
  ...
  assert.deepEqual(contract.pending, [
    ...
    {
      filename: NUTRITION_PREWORKOUT_FACTS_MIGRATION,
      sha256: NUTRITION_PREWORKOUT_FACTS_SHA256,
    },
  ]);
  ...
  assert.equal(sha256File(path.join(SOURCE, PRICE_HISTORY_REUSE_MIGRATION)), PRICE_HISTORY_REUSE_SHA256);
  ...
  assert.equal(sha256File(path.join(SOURCE, TEN_REPS_V8_SHORT_SOURCE_IDS_MIGRATION)), TEN_REPS_V8_SHORT_SOURCE_IDS_SHA256);
  ...
  assert.equal(sha256File(path.join(SOURCE, TEN_REPS_V10_SHORT_SOURCE_MIGRATION)), TEN_REPS_V10_SHORT_SOURCE_SHA256);
  assert.equal(sha256File(path.join(SOURCE, TEN_REPS_V10_LIQUID_MIGRATION)), TEN_REPS_V10_LIQUID_SHA256);
  assert.equal(sha256File(path.join(SOURCE, TEN_REPS_V10_SIBLING_MIGRATION)), TEN_REPS_V10_SIBLING_SHA256);
});
...
test("materialization preserves every original migration byte-for-byte", () => {
  ...
  assert.equal(fs.readdirSync(path.join(workdir, "supabase", "migrations")).length, 98);
...
test("production binds its exact ledger with the reviewed nutrition migrations pending", () => {
  const contract = CONTRACTS.PRODUCTION;
  const excluded = new Set(Object.keys(contract.excluded));
  const pending = new Set(contract.pending.map(({ filename }) => filename));
  ...
  assert.equal(result.ledger_fingerprint, contract.ledgerFingerprint);
  assert.equal(result.selected_files.length, 206);
  assert.deepEqual(result.pending_files, [NUTRITION_VARIANT_PROVENANCE_MIGRATION, NUTRITION_PREWORKOUT_FACTS_MIGRATION]);
  assert.equal(result.pending_file, null);
  assert.equal(result.pending_sha256, null);
  ...
  assert.ok(result.selected_files.includes(PRICE_HISTORY_REUSE_MIGRATION));
  ...
  assert.ok(result.selected_files.includes(TEN_REPS_V8_SHORT_SOURCE_IDS_MIGRATION));
  assert.ok(result.selected_files.includes(TEN_REPS_V8_SIBLING_VARIANTS_MIGRATION));
...
test("staging excludes the production-only exact-pack migrations byte-for-byte", () => {
  ...
  for (const filename of [
    ...
    EXPIRED_DISCOUNT_JONS_MIGRATION,
    SERIALIZED_SHARED_REFRESH_MIGRATION,
    PRICE_HISTORY_REUSE_MIGRATION,
  ]) {
