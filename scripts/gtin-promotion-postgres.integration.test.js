...
test("guarded GTIN promotion runs atomically on disposable PostgreSQL", { skip: !dockerIsAvailable && "Docker daemon unavailable" }, () => {
  ...
  try {
    ...
    const targetVariants = variants.map((row) => `(${row.id},${row.product_id},'variant-${row.id}',${sqlLiteral(row.display_name)},${sqlLiteral(row.flavour_code)},${sqlLiteral(row.flavour_label)},${row.size_value},${sqlLiteral(row.size_unit)},${row.pack_count},${sqlLiteral(row.product_format)},true,false)`).join(",");
