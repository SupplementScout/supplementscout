...
declare
  ...
  v_parent constant uuid:='720ff2b5-358a-4c15-b1c1-49262da8d254';
...
begin
  if current_user<>'postgres' or v_target->>'target_environment'<>'PRODUCTION'
    ...
    raise exception 'sequential approver repair target mismatch';
  end if;
  ...
  perform 1 from public.retailer_catalogue_parent_plans p where p.id=v_parent
    and p.parent_plan_fingerprint='1c7a638ae69a308601f2598c5ed6886b59b2c44c1861b2722c3460dcfb014754'
    and p.retailer_id=14 and p.target_environment='PRODUCTION' and p.status='PLANNED'
    and p.approval_id is null and p.approval_expires_at is null
    and p.code_commit='45fe56efd6cc90080f4c476614956f14a719e6bf'
    and p.source_snapshot_fingerprint='4ac2def92a63eb99924a8481a2ac48050b85ee234d09f39f72f6b4f14f0e62a0'
    and p.source_captured_at='2026-09-19T19:57:09.527Z'::timestamptz
    and p.created_by='github:SupplementScout/supplementscout:35465811784:1' for update;
  if not found then raise exception 'empty 10 Reps parent precondition mismatch'; end if;
  if (select count(*) from public.retailer_catalogue_child_plans where parent_plan_id=v_parent)<>19
     or (select count(*) from public.retailer_catalogue_child_plans where parent_plan_id=v_parent and status='PLANNED')<>19
     or (select coalesce(sum(jsonb_array_length(record_ids)),0) from public.retailer_catalogue_child_plans where parent_plan_id=v_parent and status='PLANNED')<>935
     ...
     or exists(select 1 from public.retailer_offer_sync_batch_approvals a join public.retailer_catalogue_child_plans c on c.id=a.child_plan_id where c.parent_plan_id=v_parent) then
    raise exception 'empty 10 Reps child/run/approval precondition mismatch';
  end if;
  update public.retailer_catalogue_child_plans set status='SUPERSEDED',updated_at=v_now,
    audit_log=audit_log||jsonb_build_array(jsonb_build_object('event','EMPTY_REFRESH_SUPERSEDED','at',v_now,'authority','owner-directed-chat-2026-09-19-complete-retailer-refreshes'))
  where parent_plan_id=v_parent and status='PLANNED';
  get diagnostics v_rows=row_count;
  if v_rows<>19 then raise exception 'empty 10 Reps child cleanup affected % rows',v_rows; end if;
  update public.retailer_catalogue_parent_plans set status='SUPERSEDED',updated_at=v_now,
    audit_log=audit_log||jsonb_build_array(jsonb_build_object('event','EMPTY_REFRESH_SUPERSEDED','at',v_now,'authority','owner-directed-chat-2026-09-19-complete-retailer-refreshes','business_writes',0))
  where id=v_parent and status='PLANNED';
  get diagnostics v_rows=row_count;
  if v_rows<>1 or public.retailer_catalogue_business_counts() is distinct from v_before
     or (select count(*) from public.retailer_catalogue_child_plans where parent_plan_id=v_parent and status='SUPERSEDED')<>19 then
    raise exception 'empty 10 Reps cleanup postcondition mismatch';
  end if;
end
$cleanup$;
