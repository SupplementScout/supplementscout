...
begin
  if current_user <> 'postgres'
    ...
    raise exception 'Jon''s ready grams batch 3 evidence or target mismatch';
  end if;
  if jsonb_array_length(v_scope) <> 4
     or (select count(distinct (x->>'product_id')::bigint) from jsonb_array_elements(v_scope) x) <> 4
     or (select count(distinct (x->>'mapping_id')::bigint) from jsonb_array_elements(v_scope) x) <> 4
     or (select count(distinct (x->>'offer_id')::bigint) from jsonb_array_elements(v_scope) x) <> 4
     or exists(select 1 from jsonb_array_elements(v_scope) x where x->>'size_unit'<>'g') then
    raise exception 'Jon''s ready grams batch 3 scope mismatch';
  end if;
  ...
  if v_exact_before<>435 then raise exception 'Jon''s exact-pack baseline is %, expected 435',v_exact_before; end if;
  ...
  if (select count(*) from public.products)<>v_products_before
    ...
    raise exception 'Jon''s ready grams batch 3 global postcondition mismatch';
  end if;
end
$apply$;
