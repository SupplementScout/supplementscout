...
**Better-value alternatives status:** MVP is `LIVE VERIFIED` at commit `8532613` on existing product pages. It reuses the shared 24-hour freshness classifier, complete delivered prices, exact-variant resolution, category normalization, verified unit-value helpers and consent-aware analytics. One section uses one shared basis, contains at most three lower-value same-category/exact-format products, and fails closed for stale, OOS, unresolved, incomplete-delivery or incomparable candidates. Public postflight on CREA-4 rendered three canonical internal alternatives on the shared per-serving basis; all three destinations returned `200` with their expected canonicals. The stale active Rich Piana CreaTen page remained `200` with `Availability being rechecked`, no current price, no schema offers, no retailer CTA and no alternatives section. No route, catalogue write, personalization or nutrition-equivalence claim was added. Next product evidence is section impression/click and downstream retailer-offer CTR; the final remaining step in the owner-selected catalogue package is Predators Gear source proof under its unchanged approval gate.
...
**3 September 2026 eBay exact-237 freshness checkpoint:** scheduled run
`33742461042` completed safe execution and DB postflight for `188` freshness-only
plans and isolated `49` rows (`11` price, `23` mapping/default-variant, `7`
source identity-quality, `8` HTTP `404`). The workflow failed only at evidence
sealing because its plan comparison included the intentionally changed
`last_checked_at`. The local repair binds per-row plans to the captured baseline
and permits only that freshness transition; archived evidence then returns
`PASS` and price drift still blocks. The 49 rows remain real backlog: no price,
...
It is bound to run `33773580580`, artifact `9900823759`, the sealed artifact
