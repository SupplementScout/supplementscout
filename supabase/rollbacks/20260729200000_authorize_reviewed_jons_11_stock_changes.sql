...
begin
  if current_user<>'postgres'
    ...
    raise exception 'Jon''s 11-change reviewed rollback requires production database owner';
  end if;
  if not exists(
    select 1
    ...
    raise exception 'Jon''s 11-change reviewed authorization is not installed';
  end if;
  if exists(
    select 1
    ...
    raise exception 'rollback is forbidden after any Jon''s 11-change reviewed binding';
  end if;
end
$preflight$;
