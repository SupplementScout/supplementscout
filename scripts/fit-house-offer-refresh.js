...
const DATABASE_CONNECTION_TIMEOUT_MS=15000;
const DATABASE_QUERY_TIMEOUT_MS=120000;
const SUPABASE_PAGE_TIMEOUT_MS=30000;
...
function parseArgs(argv){const out={};for(const arg of argv){const m=arg.match(/^--([^=]+)=(.*)$/);if(!m||out[m[1]]!==undefined||!["target","mode","isolate-unsafe","scope-segment"].includes(m[1]))throw new Error(`invalid argument ${arg}`);out[m[1]]=m[2]}if(!TARGETS[out.target]||!["dry-run","apply"].includes(out.mode))throw new Error("required --target=staging|production --mode=dry-run|apply");if(out["isolate-unsafe"]!==undefined&&!['true','false'].includes(out["isolate-unsafe"]))throw new Error("isolate-unsafe must be true or false");out.isolateUnsafe=out["isolate-unsafe"]==="true";const scopeSegment=out["scope-segment"]||null;delete out["isolate-unsafe"];delete out["scope-segment"];if(scopeSegment)out.scopeSegment=scopeSegment;return out}
function loadEnvFile(file){if(!fs.existsSync(file))return{};const out={};for(const line of fs.readFileSync(file,"utf8").split(/\r?\n/)){const m=line.replace(/^\uFEFF/,"").match(/^\s*([A-Z0-9_]+)\s*=\s*(.*)$/);if(m)out[m[1]]=m[2].trim().replace(/^(['"])(.*)\1$/,"$2")}return out}
function loadEnvironment(){const files=[path.join(ROOT,".env.local")];if(config.local_env_path){invariant(/^\.env\.[a-z0-9.-]+\.local$/.test(config.local_env_path)||config.local_env_path==="../supplementscout.env.10reps.local","invalid local env path");files.push(path.resolve(ROOT,config.local_env_path))}for(const file of files)Object.assign(process.env,Object.fromEntries(Object.entries(loadEnvFile(file)).filter(([key])=>!process.env[key])))}
...
function enforceConfirmationOnly(run,setting){
  invariant(setting===undefined||setting===""||setting==="false"||setting==="true","invalid confirmations-only setting");
  if(setting!=="true")return;
  invariant(run.classification.rows.length===config.approved_mapping_count
    &&run.classification.rows.every(row=>row.action==="VERIFY_NO_CHANGE")
    &&(run.classification.quarantined_rows||[]).length===0
    &&run.artifacts.flatMap(artifact=>artifact.rows).length===config.approved_mapping_count,
...
function loadApprovedManifest(){
  const file=path.join(ROOT,config.manifest_path),bytes=fs.readFileSync(file),actual=crypto.createHash("sha256").update(bytes.toString("utf8").replace(/\r\n/g,"\n")).digest("hex");
  ...
  if(config.manifest_mode==="authority_fingerprint"){
    ...
    if(manifest.scope_segments){const segmentIds=Object.values(manifest.scope_segments).flat().map(String),manifestIds=manifest.rows.map(row=>String(row.offer_id));invariant(segmentIds.length===config.approved_mapping_count&&new Set(segmentIds).size===segmentIds.length&&segmentIds.every(id=>manifestIds.includes(id)),"approved manifest scope segment drift")}
  }
...
async function readSourceSnapshot(capturedAt){
  if(config.source_platform==="CSV_PRODUCT_FEED"){
    const feedUrl=process.env[config.source_url_env];invariant(feedUrl,`${config.source_url_env} is required`);
    return readCsvProductFeed({feedUrl,storeUrl:config.store_url,capturedAt,timeoutMs:config.source_fetch.timeout_ms,maximumBytes:config.source_fetch.maximum_bytes,maximumAttempts:config.source_fetch.maximum_attempts,retryBaseDelayMs:config.source_fetch.retry_base_delay_ms});
  }
  if(config.source_platform==="WOOCOMMERCE_PRODUCT_PAGES"){
    const approved=loadApprovedManifest().manifest;
    return readWooCommerceMappedSnapshot({storeUrl:config.store_url,manifestRows:approved.rows,expectedCount:config.approved_mapping_count,capturedAt,timeoutMs:config.source_fetch.timeout_ms,maximumAttempts:config.source_fetch.maximum_attempts,retryBaseDelayMs:config.source_fetch.retry_base_delay_ms,userAgent:config.source_fetch.user_agent,allowedPathPrefixes:config.source_fetch.allowed_path_prefixes});
  }
...
function mappedOfferSourceFingerprint(records,sourceVariants){
  ...
  return canonicalHash(records.map(record=>{
    ...
    if(!source)return{offer_id:String(record.offer.id),external_product_id:String(record.mapping.external_product_id),external_variant_id:String(record.mapping.external_variant_id),source_absent:true};
...
async function all(client,table,columns,filter){const out=[];for(let from=0;;from+=1000){let query=client.from(table).select(columns).range(from,from+999);if(filter)query=filter(query);const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),SUPABASE_PAGE_TIMEOUT_MS);try{const{data,error}=await query.abortSignal(controller.signal);if(error)throw error;out.push(...(data||[]));if(!data||data.length<1000)return out}finally{clearTimeout(timer)}}}
...
function scopeSegmentSummary(manifest,classification,artifacts,executed=false){if(!manifest.scope_segments)return undefined;const executableIds=new Set(artifacts.flatMap(artifact=>artifact.rows.map(row=>String(row.offer_id)))),reviewIds=new Set((classification.quarantined_rows||[]).map(row=>String(row.offer_id))),actionByOffer=new Map((classification.rows||[]).map(row=>[String(row.offer_id),row.action]));return Object.fromEntries(Object.entries(manifest.scope_segments).map(([name,values])=>{const ids=values.map(String),actions={};for(const id of ids){const action=actionByOffer.get(id);if(action)actions[action]=(actions[action]||0)+1}return[name,{approved_mapping_count:ids.length,executable_plan_count:ids.filter(id=>executableIds.has(id)).length,executed_plan_count:executed?ids.filter(id=>executableIds.has(id)).length:0,review_row_count:ids.filter(id=>reviewIds.has(id)).length,blocked_row_count:0,classification:actions}]}))}
function selectApprovedScopeSegment(rows,classification,manifest,scopeSegment,reviewed=false){if(!scopeSegment)return rows;invariant(!reviewed,"reviewed execution cannot select an approved scope segment");const approved=manifest.scope_segments?.[scopeSegment];invariant(Array.isArray(approved)&&approved.length>0,`unknown approved scope segment ${scopeSegment}`);const approvedIds=new Set(approved.map(String)),selected=rows.filter(row=>approvedIds.has(String(row.offer_id))),reviewIds=new Set((classification.quarantined_rows||[]).map(row=>String(row.offer_id)));invariant(selected.length+approved.filter(id=>reviewIds.has(String(id))).length===approved.length,"approved scope segment execution coverage drift");return selected}
...
function balancedExecutionBatches(rows,maximumBatchSize=50,maximumNewOosPerBatch=3,confirmedPriceOfferIds=[]){invariant(Number.isInteger(maximumBatchSize)&&maximumBatchSize>0,"invalid maximum batch size");invariant(Number.isInteger(maximumNewOosPerBatch)&&maximumNewOosPerBatch>0,"invalid new OOS batch limit");invariant(Array.isArray(confirmedPriceOfferIds),"invalid confirmed price offer ids");if(rows.length===0)return[];const confirmedPriceIds=new Set(confirmedPriceOfferIds.map(String));for(const offerId of confirmedPriceIds)invariant(rows.some(row=>String(row.offer_id)===offerId&&Boolean(row.changed_fields?.price)),"confirmed price scope drift");const newOos=rows.filter(row=>Boolean(row.atomic_plan.expected_state.offer.in_stock)&&!Boolean(row.atomic_plan.offer.values.in_stock)),count=Math.max(Math.ceil(rows.length/maximumBatchSize),Math.ceil(newOos.length/maximumNewOosPerBatch)),batches=Array.from({length:count},()=>[]),priceChanged=[],otherChanged=[],existingOos=[],unchanged=[];for(const row of rows){const before=Boolean(row.atomic_plan.expected_state.offer.in_stock),after=Boolean(row.atomic_plan.offer.values.in_stock);if(!after&&before)continue;if(Boolean(row.changed_fields?.price))priceChanged.push(row);else if(row.action!=="VERIFY_NO_CHANGE")otherChanged.push(row);else if(!after)existingOos.push(row);else unchanged.push(row)}let cursor=0;for(const group of[newOos,priceChanged,otherChanged,existingOos,unchanged])for(const row of group){while(batches[cursor%count].length>=maximumBatchSize)cursor+=1;batches[cursor%count].push(row);cursor+=1}for(const batch of batches){batch.sort((a,b)=>Number(a.offer_id)-Number(b.offer_id));const unconfirmedPriceCount=batch.filter(row=>Boolean(row.changed_fields?.price)&&!confirmedPriceIds.has(String(row.offer_id))).length;invariant(batch.length>0&&batch.length<=maximumBatchSize,"balanced batch size drift");invariant(batch.filter(row=>Boolean(row.atomic_plan.expected_state.offer.in_stock)&&!Boolean(row.atomic_plan.offer.values.in_stock)).length<=maximumNewOosPerBatch,"new OOS batch limit exceeded");invariant(unconfirmedPriceCount/batch.length<Number(config.guardrails.mass_price_change_block_ratio),"balanced batch price-change ratio drift")}invariant(batches.reduce((sum,batch)=>sum+batch.length,0)===rows.length,"balanced batch coverage drift");return batches}
...
function requireAuditedMissingOwnerApproval(auditedMissing,reconciled,classification,reviewed,ownerApprovedMissing=null){
  ...
  const missingVariantIds=new Set(reconciled.missingVariantIds.map(String));
  const newlyUnavailable=(classification.rows||[]).filter(row=>row.target.in_stock&&!row.source.in_stock&&missingVariantIds.has(String(row.external_variant_id)));
...
function authorizeOwnerApprovedMissingVariant(classification,ownerApprovedMissing){
  ...
  invariant(classification.reason==="MASS_OOS"&&classification.rows.length===286
    ...
    &&classification.guard_evidence.current_oos<=103
...
async function readState(target){
  ...
  }else{const connection=roleCredential(target,"validator");const client=new Client({connectionString:connection,ssl:{rejectUnauthorized:false},application_name:`${config.retailer_slug}-offer-refresh-${target}-read`,connectionTimeoutMillis:DATABASE_CONNECTION_TIMEOUT_MS,query_timeout:DATABASE_QUERY_TIMEOUT_MS,keepAlive:true,options:`-c default_transaction_read_only=on -c statement_timeout=${DATABASE_QUERY_TIMEOUT_MS}`});await client.connect();try{await client.query("begin read only");await client.query(`select set_config('app.retailer_catalogue_${target}_marker','1',true),set_config('app.retailer_catalogue_allow','1',true)`);await client.query(`set role retailer_catalogue_${target}_validator`);invariant((await client.query("select current_user,current_setting('transaction_read_only') ro")).rows[0].current_user===`retailer_catalogue_${target}_validator`,"read role mismatch");[retailers,products,variants,mappings,offers,history]=await Promise.all([
...
function sourceFor(record,sourceByVariant){const source=sourceByVariant.get(String(record.mapping.external_variant_id));invariant(source,"missing mapped source variant");invariant(String(source.external_product_id)===String(record.mapping.external_product_id),"source product relationship drift");const url=config.guardrails.preserve_existing_urls?new URL(record.offer.url):new URL(`/products/${source.product_handle}`,config.store_url);if(!config.guardrails.preserve_existing_urls)url.searchParams.set("variant",String(source.external_variant_id));const shipping=config.shipping_policy.mode==="preserve_existing_reviewed"?money(record.offer.shipping_cost):source.shipping_cost;return{...source,shipping_cost:shipping,url:url.href,external_url:config.guardrails.preserve_existing_urls?record.mapping.external_url:url.href,total_price:(Number(source.price)+Number(shipping||0)).toFixed(2)}}
...
function classificationDiagnostic(classification){
  ...
  return{
    ...
    deferred_changed_offer_ids:classification.deferred_changed_offer_ids||[],
...
function applyOwnerApprovedMissingVariantGuardBaseline(guard,authorization){
  ...
  invariant(authorization.manifest_sha256==="613575ec4fc1dfb989c0113c67032d9f5ad30a24c68c23f87b6b04969a3b23a7"
    &&authorization.global_total_oos_count===103
...
async function buildRun(target,state,diagnostic=null,reviewed=null,isolateUnsafe=false,scopeSegment=null){
  ...
  if(!reviewed&&isolateUnsafe){
    ...
    if(aggregateReasons.has(classification.reason)||hardPriceOfferIds.length){
      const firstMappedFingerprint=mappedOfferSourceFingerprint(state.records,reconciled.sourceVariants),secondCapturedAt=freshCapturedAt();
...
function validationGuardSummary(request,batchIndex){const guard=request.guardrails;return{batch_index:batchIndex,batch_size:request.artifact.rows.length,new_oos_count:guard.new_oos_count,total_oos_count:guard.total_oos_count,previous_oos_count:guard.previous_oos_count,changed_row_count:guard.changed_row_count,limits:{maximum_new_oos_count:guard.limits.maximum_new_oos_count,maximum_oos_increase_ratio:guard.limits.maximum_oos_increase_ratio,maximum_total_oos_ratio:guard.limits.maximum_total_oos_ratio,maximum_changed_record_ratio:guard.limits.maximum_changed_record_ratio}}}
function safeRetailerCatalogueError(error){
  if(error?.code!=="P0001")return{validator_code:null,validator_summary:"validator rejected the batch"};
  try{
    const parsed=JSON.parse(error.message);
    const code=typeof parsed.code==="string"&&/^RSBI_[A-Z0-9_]{1,80}$/.test(parsed.code)?parsed.code:null;
    const summary=typeof parsed.summary==="string"&&/^[A-Za-z0-9 ,._:()'-]{1,200}$/.test(parsed.summary)?parsed.summary:"validator rejected the batch";
    return{validator_code:code,validator_summary:summary};
  }catch{return{validator_code:null,validator_summary:"validator rejected the batch"}}
}
function safeValidatorResult(result){return{valid:result?.valid===true,status:typeof result?.status==="string"&&/^[A-Z_]{1,40}$/.test(result.status)?result.status:null,row_count:Number.isInteger(Number(result?.row_count))&&Number(result.row_count)>=0?Number(result.row_count):null}}
async function validate(run,diagnostic=null){const outputs=[];for(const [index,artifact] of run.artifacts.entries()){const request=validationRequest(run,artifact),summary=validationGuardSummary(request,index+1);if(diagnostic)(diagnostic.validation_guard_batches||=[]).push(summary);try{const call=await roleCall(run.target,"validator",true,client=>client.query("select public.validate_retailer_offer_sync_batch_read_only($1::jsonb) result",[request]));const result=call.result.rows[0].result;if(!(result?.valid===true&&result.status==="DRY_RUN_VALIDATED"&&Number(result.row_count)===artifact.rows.length))throw new RefreshError("VALIDATOR_INVALID_RESULT","read-only validator returned an invalid batch result","VALIDATOR",{...summary,validator_result:safeValidatorResult(result)});outputs.push({request,result,identity:call.identity})}catch(error){if(error instanceof RefreshError)throw error;throw new RefreshError("VALIDATOR_REJECTED","read-only validator rejected an exact offer batch","VALIDATOR",{...summary,...safeRetailerCatalogueError(error)})}}return outputs}
...
async function executeRefresh(args,diagnostic,reviewed=null){
  ...
  const approvedManifest=loadApprovedManifest().manifest;
  ...
  const run=await buildRun(args.target,before,diagnostic,reviewed,args.isolateUnsafe,args.scopeSegment);
  enforceConfirmationOnly(run,process.env[`${config.phase_prefix}_CONFIRMATIONS_ONLY`]);
  ...
  if(run.artifacts.length===0){
    const output={result:"PASS_WITH_REVIEW",mode:args.mode,target:args.target,project_ref:spec.ref,approved_mapping_count:config.approved_mapping_count,execution_offer_ids:[],stock_change_offer_ids:[],verification_offer_ids:[],deferred_changed_offer_ids:run.classification.deferred_changed_offer_ids||[],executable_plan_count:0,executed_plan_count:0,review_row_count:reviewRows.length,blocked_row_count:0,scope:{mappings:config.approved_mapping_count,offers:config.approved_mapping_count,children:0,rows:0},approved_scope_segments:scopeSegmentSummary(approvedManifest,run.classification,run.artifacts),classification:counts,review_rows:reviewRows,validator_batches:0,business:{products_delta:0,variants_delta:0,mappings_delta:0,offers_delta:0,price_history_delta:0,offers_refreshed:0},safe_update:"unset"};
    write(`${args.target}-${args.mode}.json`,output);
    return output;
  }
  const validations=await validate(run,diagnostic);
  ...
  const output={...base,executed_plan_count:appliedRows,approved_scope_segments:scopeSegmentSummary(approvedManifest,run.classification,run.artifacts,true),registration:registered.result,executions:executions.map(row=>row.result),business:{products_delta:0,variants_delta:0,mappings_delta:0,offers_delta:0,price_history_delta:historyDelta,offers_refreshed:appliedRows},recovery_calls:0};
