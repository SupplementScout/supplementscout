...
const { buildCreatineCoverageReport } = require("./lib/creatine-coverage");
...
async function readAllProducts(client) {
  ...
  for (let from = 0;; from += 1000) {
    const { data, error } = await client.from("products")
      .select("id,name,slug,brand,category,product_format,net_weight_g,net_volume_ml,unit_count,unit_type,serving_count_verified,serving_size_g,serving_size_ml,creatine_per_serving_g,nutrition_verified,unit_pricing_verified,is_active,merged_into_product_id")
      .order("id")
      .range(from, from + 999);
...
async function runCli(argv = process.argv.slice(2), dependencies = {}) {
  ...
  const report = buildCreatineCoverageReport(await readAllProducts(client));
