...
async function register(run,request){const call=await roleCall(run.target,"validator",false,client=>client.query(`select public.${config.registration_rpc}($1::jsonb) result`,[request])),result=call.result.rows[0].result;invariant(result.status==="REGISTERED"&&Number(result.mapping_count)===config.approved_mapping_count&&Number(result.child_count)===run.artifacts.length&&Number(result.business_writes)===0,"registration failed");return{result,identity:call.identity}}
...
module.exports={RefreshError,authorizeReviewedMassOos,balancedExecutionBatches,buildRun,canonicalHash,classificationDiagnostic,diagnosticTemplate,executionRow,guardrailsFor,loadApprovedManifest,loadReviewedMassOosManifest,migrationBinding,parseArgs,projectSourceVariants,readState,reconcileMissingMappedVariants,registrationRequest,runWithDiagnostic,sourceHealth,sumDeltas,verificationRecord};
