...
async function register(run,request){const call=await roleCall(run.target,"validator",false,client=>client.query("select public.register_fit_house_offer_sync_control_plan($1::jsonb) result",[request])),result=call.result.rows[0].result;invariant(result.status==="REGISTERED"&&Number(result.mapping_count)===286&&Number(result.child_count)===run.artifacts.length&&Number(result.business_writes)===0,"registration failed");return{result,identity:call.identity}}
