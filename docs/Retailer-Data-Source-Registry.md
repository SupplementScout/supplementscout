...
- Routine automation is implemented in `.github/workflows/six-pack-offer-refresh.yml`. One shared expandable manifest drives the complete retailer scope; the workflow runs daily at `03:17 UTC` and supports manual dry-run/apply. Every run tests contracts, fetches the 27 exact WooCommerce product pages for all 78 approved variants, validates source identity and full manifest coverage, applies only approved existing mappings through separate production approver/executor roles, takes a fresh source capture, requires idempotency and uploads evidence.
...
- A further 35 existing canonical variants were added in one protected rollout, without creating products or variants. Run [`30283296805`](https://github.com/SupplementScout/supplementscout/actions/runs/30283296805) passed preflight, exact-scope execution, production verification and 35/35 idempotency. The single shared manifest now covers 78 offers across 27 product pages with SHA-256 `00097844976ec31f6c1cf0bfd3c9a4e8abcd9c0af3d2b28aade17688a2c1046e`; a local fresh-source check returned 78 `VERIFY_NO_CHANGE`, zero changes and zero blockers before activation.
