...
| SEO-15 | P1 | Launch a data-backed deals and price-drops page. | `BLOCKED` | Stage 1 and Indexability Lifecycle P0 are live verified. Jon's runs progressed proven series/confirmations from `418/506` (`32812270590`) to `503/506` (`32915426696`). The 26 August readiness audit covered all `1,801` approved mappings/offers of the other six producers: `1,120` were exact-ready and `681` failed closed at capture. Owner-approved GYM HIGH producer enablement is live for its exact `66`-offer scope; guarded alignments increased it to `50` ready, with only 16 apparel/accessory identities intentionally blocked. Fit House remains disabled; five guarded alignments increased readiness from `166/286` to `253/286`. Its remaining 33 rows stay fail-closed: 26 are absent from the current source and seven need separate serving or canonical-identity resolution, including flavour conflicts `2095` and `2096`. The owner decision for Sodium Butyrate mapping `869` is recorded for a later separately sealed package, not silently added to the applied batch. The other four producers remain disabled. No GYM HIGH or Fit House workflow was manually run. Blocker: the minimum 14-day identity-proven history cannot exist before 8 September 2026; the recommended first publication decision is after 24 September. Stage 3/public price-drop claims remain disabled. Mandatory return to SEO-15 is preserved. |
...
- The corrective lifecycle implementation shipped in commit
  `1f7bfc08075899849f22f5bf80b978fe7cb60de3`, `Stabilize and document SEO
  indexability lifecycle`, across `58` files. Vercel production deployment
  `2bmj7eiPXnL3pNmCcy8av3TyCg6f` succeeded and the public live verification
  passed.
- All 15 approved routes returned HTTP `200`, emitted `index, follow`, used the
  correct self-canonical and occurred exactly once in the sitemap:
  `/deals`, `/whey-protein`, `/vegan-protein`, `/protein-bars`,
  `/whey-isolate`, `/mass-gainer`, `/pre-workout`, `/amino-acids`,
  `/multivitamins`, `/creatine`, `/hydration`,
  `/brands/applied-nutrition`, `/brands/per4m`, `/brands/biotech-usa` and
  `/retailers/ebay-uk`.
- Parameter URLs remained `noindex, follow` with the base-route canonical.
  SEO-04 remained unchanged. The sitemap contained `1,096` unique URLs,
  including `1,070` product URLs, with `0` duplicates and no GYM HIGH URL.
- Normal reads produced no false empty lists and no HTTP 5xx responses. The
  shared cache shortened subsequent reads. Homepage, a representative product,
  About, How we compare, empty search and `robots.txt` had no regression.
- The deterministic Guardian fixture correction shipped in commit
  ...
  `32737495783` succeeded; Full job `97463732331` succeeded and Integration job
  `97463733544` was correctly skipped. Vercel deployment
  `ZVxadQiAwXhgL4xfC36cw7JKcrF4` succeeded.
- The release and verification ran zero refresh workflows and performed zero
  production-data writes. GYM HIGH remains `owner_deferred`. SEO-15 remains
  `IN PROGRESS`: Stage 1 and this corrective P0 are live verified, while Stage
  2 and Stage 3 are not started. The binding sequence remains SEO-15, SEO-16,
  SEO-17.
...
- The owner accepted the preceding Protein P0 as completed and production
  ...
  write was performed while resuming `SEO-13`.
- A production read-only audit at `2026-08-23T13:28:18.905Z` found `52`
  active, unmerged canonical products in exact category `Protein Bars`, `183`
  active variants and `187` embedded offers. The reviewed bar/wafer/flapjack
  boundary, active concrete-variant binding and consistent known-pack rule
  reduce that source set to `10` eligible products.
- Inside the unchanged 24-hour boundary, `4` products expose `27` fresh
  exact-pack offers from `3` retailers. `3` products have at least two current
  retailers, and the two comparison retailers are Whey Okay and eBay UK. The
  gate therefore passes at `3 / 2 / 27` against the required `3 / 2 / 20`.
  All `27` included offers have a known delivered price.
- The four visible exact packs are Grenade Carb Killa Protein Bar `1 x 60g`,
  Optimum Nutrition Protein Crisp Bar `10 x 65g`, Clif Bar Energy Bar
  `12 x 68g` and PER4M Protein Bars Box `12 x 62g`. Mixed or unresolved pack
  families remain excluded rather than inferred.
- None of the four visible products has an approved product-level verified
  unit count. The page therefore shows exact pack identity and delivered price
  but deliberately hides price per bar and nutrition-dependent value metrics.
- `/protein-bars` reuses the central comparison normalizer, exact variant
  bindings, delivered-price treatment, freshness presentation, consent-aware
  outbound analytics, dynamic readiness sitemap gate and existing metadata and
  structured-data patterns. It adds one bounded URL, not a generator or new
  catalogue variant.
