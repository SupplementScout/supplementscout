...
begin
  ...
  if v_mapping_id is null or v_product_id is null or v_variant_id is null
     ...
     or jsonb_typeof(v_values->'external_options') <> 'object'
     or v_values->'external_options' is distinct from v_evidence->'external_options'
     or (select count(*) from jsonb_each(v_values->'external_options')) <> 2
     or not (v_values->'external_options' ? 'Size')
     or not (v_values->'external_options' ? 'Flavour')
     or nullif(v_evidence->>'size_value','') is null
     or nullif(v_evidence->>'flavour','') is null then
    return false;
  end if;
...
begin
  ...
  if not public.atomic_import_has_exact_keys(
    p_plan->'meta',
    array['version','plan_kind','operation_type','source_row_fingerprint','plan_fingerprint']
  )
  ...
  if v_variant.is_default then
    if (
      v_evidence->>'flavour' is not null
      or v_evidence->>'size_value' is not null
      or exists (
