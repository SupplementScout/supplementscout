...
async function validate(run,diagnostic=null){const outputs=[];for(const [index,artifact] of run.artifacts.entries()){const request=validationRequest(run,artifact),summary=validationGuardSummary(request,index+1);if(diagnostic)(diagnostic.validation_guard_batches||=[]).push(summary);try{const call=await roleCall(run.target,"validator",true,client=>client.query("select public.validate_retailer_offer_sync_batch_read_only($1::jsonb) result",[request]));const result=call.result.rows[0].result;invariant(result.valid&&result.status==="DRY_RUN_VALIDATED"&&Number(result.row_count)===artifact.rows.length,"validator rejected child");outputs.push({request,result,identity:call.identity})}catch(error){throw new RefreshError("VALIDATOR_REJECTED","read-only validator rejected an exact offer batch","VALIDATOR",{...summary,validator_reason:/^Read-only /.test(error.message)?error.message:"validator rejected the batch"})}}return outputs}
