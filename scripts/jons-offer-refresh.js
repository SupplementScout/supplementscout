...
function loadReviewedMissingVariantManifest(){const file=path.join(ROOT,config.reviewed_missing_variant_manifest_path),bytes=fs.readFileSync(file),digest=crypto.createHash("sha256").update(bytes).digest("hex");invariant(digest===config.reviewed_missing_variant_manifest_sha256,"Jon's reviewed missing-variant manifest hash mismatch");const manifest=JSON.parse(bytes);invariant(manifest.schema_version===1&&manifest.kind==="jons-reviewed-missing-variants-v1"&&manifest.authority==="owner-approved-chat-2026-08-10-five-jons-recommendations"&&manifest.retailer_id==="10"&&manifest.retailer_slug==="jon-s-supplements"&&manifest.row_count===3&&manifest.policy==="KEEP_CANONICAL_IDENTITY_AND_MARK_OFFER_OOS_WHILE_EXACT_SOURCE_VARIANT_IS_ABSENT"&&Array.isArray(manifest.rows)&&manifest.rows.length===3,"Jon's reviewed missing-variant manifest schema mismatch");const expected=["1398:1212:858:1284:10044164899154:50651557757266:CNP60005","1429:1243:866:1315:10032290431314:50602413949266:CNP09013","1654:1468:931:2950:10085668684114:50825200271698:"];const actual=manifest.rows.map(row=>[row.mapping_id,row.offer_id,row.canonical_product_id,row.canonical_variant_id,row.external_product_id,row.external_variant_id,row.external_sku??""].join(":")).sort();invariant(JSON.stringify(actual)===JSON.stringify(expected),"Jon's reviewed missing-variant scope mismatch");return{manifest,sha256:digest}}
function reconcileReviewedMissingVariants(records,sourceVariants,reviewed=loadReviewedMissingVariantManifest(),{isolateUnsafe=false}={}){const sourceIds=new Set(sourceVariants.map(row=>String(row.external_variant_id))),reviewedByVariant=new Map(reviewed.manifest.rows.map(row=>[row.external_variant_id,row])),synthetic=[],unapproved=[];for(const record of records){const variantId=String(record.mapping.external_variant_id);if(sourceIds.has(variantId))continue;const approved=reviewedByVariant.get(variantId);if(!approved){unapproved.push({mapping_id:String(record.mapping.id),offer_id:String(record.offer.id),external_product_id:String(record.mapping.external_product_id),external_variant_id:variantId});continue}invariant(String(record.mapping.id)===approved.mapping_id&&String(record.offer.id)===approved.offer_id&&String(record.product.id)===approved.canonical_product_id&&String(record.variant.id)===approved.canonical_variant_id&&String(record.mapping.external_product_id)===approved.external_product_id&&(record.mapping.external_sku||null)===(approved.external_sku||null),"Jon's reviewed missing variant no longer matches its canonical mapping");const direct=new URL(record.mapping.external_url),parts=direct.pathname.split("/").filter(Boolean),productHandle=parts[0]==="products"?parts[1]:null;invariant(productHandle,"Jon's reviewed missing variant URL is invalid");synthetic.push({external_product_id:approved.external_product_id,external_variant_id:approved.external_variant_id,product_handle:productHandle,external_sku:approved.external_sku,price:money(record.offer.price),shipping_cost:money(record.offer.shipping_cost),in_stock:false,source_updated_at:null,reviewed_source_absent:true})}if(unapproved.length&&!isolateUnsafe)throw new RefreshError("IDENTITY_DRIFT","unreviewed Jon's mapped variants are absent from source","CLASSIFIER",{unapproved_missing_variants:unapproved});return{sourceVariants:[...sourceVariants,...synthetic],reviewed_missing:synthetic.map(row=>row.external_variant_id),unreviewed_missing:unapproved,manifest_sha256:reviewed.sha256}}
...
function partitionIsolatedExecutionRows(rows,maximumRows=50,maximumChildren=20){
  ...
  for(const row of otherChanged){
    const rowChangesPrice=Boolean(row.changed_fields.price);
    let bucket=sensitive.find(candidate=>candidate.length<12&&(!rowChangesPrice||candidate.filter(item=>item.changed_fields.price).length<9));
  ...
  for(const bucket of sensitive){
    ...
    const priceChanges=bucket.filter(row=>row.changed_fields.price).length;
    const requiredForChangeRatio=bucket.length*3;
    const requiredForOosRatio=Math.max(0,Math.ceil(currentOos/0.35)-currentOos);
    const requiredForPriceRatio=Math.max(0,priceChanges*5+1-bucket.length);
    const required=Math.max(requiredForChangeRatio,requiredForOosRatio,requiredForPriceRatio);
...
function mappedOfferSourceFingerprint(records,sourceVariants,isolatedMissingVariantIds=new Set()){
  ...
  return canonicalHash(records.map(record=>{
    ...
    if(!source){invariant(isolatedMissingVariantIds.has(String(record.mapping.external_variant_id)),"mapped source fingerprint is missing a variant");return{offer_id:String(record.offer.id),external_product_id:String(record.mapping.external_product_id),external_variant_id:String(record.mapping.external_variant_id),source_absent:true}}
...
async function confirmAggregateSource(state,firstSourceFingerprint,diagnostic,isolatedMissingVariantIds=new Set()){
  ...
  const reconciled=reconcileReviewedMissingVariants(state.records,raw,loadReviewedMissingVariantManifest(),{isolateUnsafe:isolatedMissingVariantIds.size>0});
  const secondFingerprint=mappedOfferSourceFingerprint(state.records,reconciled.sourceVariants,isolatedMissingVariantIds);
  ...
  return{snapshot,sourceVariants:reconciled.sourceVariants,mappedFingerprint:secondFingerprint,capturedAt};
}
...
async function buildRun(target,state,diagnostic=null,reviewed=null,isolateUnsafe=false){
  ...
  const missingReconciliation=reconcileReviewedMissingVariants(state.records,sourceVariants,loadReviewedMissingVariantManifest(),{isolateUnsafe:isolateUnsafe&&!reviewed});
  const effectiveSourceVariants=missingReconciliation.sourceVariants;
  const isolatedMissingVariantIds=new Set(missingReconciliation.unreviewed_missing.map(row=>row.external_variant_id));
  if(diagnostic)diagnostic.guard_results.push({guard:"REVIEWED_MISSING_VARIANTS",result:isolatedMissingVariantIds.size?"PASS_WITH_REVIEW":"PASS",manifest_sha256:missingReconciliation.manifest_sha256,reviewed_missing:missingReconciliation.reviewed_missing,isolated_unreviewed_missing:missingReconciliation.unreviewed_missing});
  ...
  let reviewedRows=null;
  if(reviewed){
    try{reviewedRows=selectReviewedClassificationRows(classification,reviewed)}catch(error){
      throw new RefreshError("REVIEWED_MANIFEST_DRIFT",error.message,"REVIEWED_CONTRACT",{classifier_reason:classification.reason||null,reviewed_manifest_sha256:reviewed.sha256,reviewed_source_fingerprint:reviewed.manifest.source_capture_sha256,live_source_fingerprint:snapshot.semantic_source_fingerprint});
    }
    ...
    if(aggregateReasons.has(classification.reason)||hardPriceOfferIds.length){
      const firstMappedFingerprint=mappedOfferSourceFingerprint(state.records,effectiveSourceVariants,isolatedMissingVariantIds);
      const confirmation=await confirmAggregateSource(state,firstMappedFingerprint,diagnostic,isolatedMissingVariantIds);
      if(hardPriceOfferIds.length){
        classification=classifyExistingOffers({targets,sourceVariants:confirmation.sourceVariants,policy,guardScope:{name:"JONS_FULL_506",retailer:"Jon's Supplements"},sourceCapturedAt:capturedAt,now:new Date(capturedAt),sourceProductCount:confirmation.snapshot.products.length,previousSourceProductCount:config.source_baseline.product_count,reviewedPriceAnomalyOfferIds:hardPriceOfferIds,quarantineUnsafeRows:true});
        ...
        invariant(confirmedObserved.size===hardPriceOfferIds.length,"confirmed Jon's price scope changed during reclassification");
        automaticPriceConfirmation={kind:"jons-two-capture-price-confirmation-v1",first_source_fingerprint:snapshot.semantic_source_fingerprint,second_source_fingerprint:confirmation.snapshot.semantic_source_fingerprint,first_mapped_fingerprint:firstMappedFingerprint,second_mapped_fingerprint:confirmation.mappedFingerprint,second_captured_at:confirmation.capturedAt,confirmed_offer_ids:hardPriceOfferIds};
      }
    }
  ...
  discovery.reviewed_raw_missing_variants=[...mappedIds].filter(id=>!rawSourceIds.has(id));
  ...
  return{target,spec,capturedAt,snapshot,sourceVariants,classification,artifacts,manifest,manifestFingerprint:canonicalHash(manifest),binding,head,discovery,isolatedMissingVariants:[...isolatedMissingVariantIds].sort(),reviewed,reviewedExpiresAt,reviewedContract,scopedSourceEvidence,mappedSourceEvidence,isolateUnsafe,automaticPriceConfirmation};
}
...
async function executeRefresh(args,diagnostic){
  ...
  const reviewed=args["reviewed-manifest"]?loadReviewedMixedChangeManifest(args["reviewed-manifest"],args["reviewed-manifest-sha256"]):null;
  ...
  const isolatedMissing=[...run.isolatedMissingVariants].sort();
  if(run.discovery.missing_variants.length!==0&&(!args.isolateUnsafe||JSON.stringify([...run.discovery.missing_variants].sort())!==JSON.stringify(isolatedMissing)))throw new RefreshError("SOURCE_INCOMPLETE","missing mapped source identity","SOURCE_GUARD",{missing_variants:run.discovery.missing_variants});
