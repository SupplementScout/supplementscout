...
begin
  ...
  v_definition:=pg_get_functiondef(v_function);
  if position('v_changed::numeric / v_row_count::numeric > v_maximum_changed' in v_definition)=0
     or position('v_price_changed::numeric / v_row_count::numeric >= v_mass_price_ratio' in v_definition)=0 then
    raise exception 'Simply confirmed validator aggregate guard anchor mismatch';
  end if;
  v_definition:=replace(v_definition,
    'v_changed::numeric / v_row_count::numeric > v_maximum_changed',
    '(case when current_setting(''app.simply_aggregate_price_confirmation'',true)=''true'' then v_changed-v_price_changed else v_changed end)::numeric / v_row_count::numeric > v_maximum_changed');
  v_definition:=replace(v_definition,
    'v_price_changed::numeric / v_row_count::numeric >= v_mass_price_ratio',
    '(current_setting(''app.simply_aggregate_price_confirmation'',true) is distinct from ''true'' and v_price_changed::numeric / v_row_count::numeric >= v_mass_price_ratio)');
  execute v_definition;
end
$repair$;
...
grant execute on function public.validate_simply_confirmed_price_read_only(jsonb) to retailer_sync_validator;
