...
export function buildMassGainerStructuredData(rows: MassGainerComparisonRow[]) {
    ...
    "@graph": [
      {
        ...
        "@id": pageUrl,
        url: pageUrl,
        name: "Compare Mass Gainer Prices UK",
        description,
      ...
      },
      {
        "@type": "ItemList",
        "@id": itemListId,
        name: "Mass Gainer powders with recently checked UK retailer offers",
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Mass Gainer", item: pageUrl },
        ],
      },
    ],
  };
...
function MassGainerProductCard({ row, position }: { row: MassGainerComparisonRow; position: number }) {
    ...
    <article className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-700">{row.offerCount} recently checked in-stock offer{row.offerCount === 1 ? "" : "s"} from {retailerNames.join(", ")}.</p>
          <p className="mt-1 text-xs text-zinc-500">{checkedAt ? `Latest check: ${checkedAt}` : "Check time unavailable"}</p>
        </div>
...
export function MassGainerPageContent({ result }: { result: MassGainerComparisonResult }) {
  const latestCheck = formatCheckedAt(result.summary.latestOfferCheckedAt);
  const jsonLd = buildMassGainerStructuredData(result.rows);
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-950">
      <CategoryViewAnalytics category="Mass Gainer" sourcePage="mass_gainer_comparison" />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} />
      <header className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6"><Link href="/" className="text-xl font-bold">SupplementScout</Link><Link href="/search?q=mass%20gainer" className="text-sm font-semibold text-zinc-700 hover:text-zinc-950">Search Mass Gainer</Link></div>
      </header>
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        <nav aria-label="Breadcrumb" className="text-sm text-zinc-600"><ol className="flex items-center gap-2"><li><Link href="/">Home</Link></li><li aria-hidden="true">/</li><li aria-current="page">Mass Gainer</li></ol></nav>
        <div className="mt-6 max-w-4xl">
          <p className="text-sm font-semibold uppercase tracking-wide text-zinc-500">UK retailer price comparison</p>
          <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">Compare Mass Gainer Prices UK</h1>
          <p className="mt-5 text-base leading-7 text-zinc-700 sm:text-lg">Compare recently checked offers for products reviewed as Mass Gainer powders. Known delivery is included when available; missing delivery, serving and nutrition values are never estimated.</p>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-12 sm:px-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><h2 className="text-2xl font-bold">Current Mass Gainer comparison</h2><p className="text-sm text-zinc-600">{latestCheck ? `Latest retailer check: ${latestCheck}` : "No current check time available"}</p></div>
        ...
        {result.error && <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-6"><h2 className="text-xl font-bold">Current Mass Gainer data is temporarily unavailable</h2><p className="mt-2">No old prices have been substituted.</p></div>}
        {!result.error && result.rows.length === 0 && <div className="mt-6 rounded-xl border border-zinc-200 bg-white p-6"><h2 className="text-xl font-bold">No recently checked Mass Gainer offers</h2><p className="mt-2 text-zinc-600">Older prices remain hidden until retailer data is checked again.</p></div>}
        {result.rows.length > 0 && <div className="mt-6 space-y-4">{result.rows.map((row, index) => <MassGainerProductCard key={row.id} row={row} position={index + 1} />)}</div>}
      </section>
      <section className="border-y border-zinc-200 bg-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-2">
          <div><h2 className="text-2xl font-bold">Reviewed inclusion boundary</h2><p className="mt-3 leading-7 text-zinc-700">Only active, unmerged canonical products in the reviewed Mass Gainer category with a confirmed powder format are included. Generic protein powders, ready-to-drink shakes, bars and products with unresolved formats stay outside this comparison.</p></div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <h2 className="text-2xl font-bold">Mass Gainer comparison questions</h2>
