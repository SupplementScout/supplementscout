...
function sourceProduct(item, overrides = {}) {
  return {
    ...
    vendor: item.brand === "7Nutrition" ? "7 Nutrition" : item.brand === "GYM HIGH" ? "Gym High" : item.brand,
...
test("generated rows preserve the approved five-batch mapping order", () => {
  ...
  assert.deepEqual(
    result.rows.slice(52, 72).map((row) => row.external_product_id),
    [
  ...
  assert.deepEqual(result.rows.slice(72).map((row) => row.external_product_id), ["9673951019248"]);
});
...
test("mapping 73 is the exact existing canonical Shred Mode addition", () => {
  const mapping = config.products[72];
  assert.equal(mapping.canonical_product_id, 508);
  assert.equal(mapping.shopify_product_id, "9673951019248");
  assert.equal(mapping.shopify_variant_id, "48121139658992");
  assert.equal(mapping.expected_handle, "gym-high-shred-mode-60-capsules");
  assert.equal(mapping.canonical_slug, "gym-high-shred-mode-60-capsules");
  assert.equal(mapping.approved_price, 39.99);
  assert.equal(mapping.approved_in_stock, true);

  for (const [field, value] of [
    ["canonical_product_id", 509],
    ["canonical_name", "Changed Shred Mode"],
    ["canonical_slug", "changed-shred-mode"],
    ["shopify_product_id", "1"],
    ["shopify_variant_id", "2"],
    ["expected_handle", "changed-shred-mode"],
    ["approved_price", 40],
    ["approved_in_stock", false],
  ]) {
    const changed = structuredClone(config);
    changed.products[72][field] = value;
    assert.throws(() => validateConfig(changed), /Shred Mode existing canonical mapping/);
  }
});
...
test("batch offer reporting splits pre-apply and post-apply actions by canonical slug", () => {
  const before = batchOfferCounts(
    config,
    rowLevelActions(Array(10).fill("unchanged"), Array(12).fill("unchanged"), Array(16).fill("unchanged"), Array(14).fill("unchanged"), Array(20).fill("unchanged"), "create")
  );
  ...
  assert.deepEqual(before.batch_5, { offers_created: 0, offers_updated: 0, offers_unchanged: 20 });
  assert.deepEqual(before.existing_canonical_additions, { offers_created: 1, offers_updated: 0, offers_unchanged: 0 });

  const after = batchOfferCounts(
    config,
    rowLevelActions(Array(10).fill("unchanged"), Array(12).fill("unchanged"), Array(16).fill("unchanged"), Array(14).fill("unchanged"), Array(20).fill("unchanged"), "unchanged")
  );
  ...
  assert.deepEqual(after.existing_canonical_additions, { offers_created: 0, offers_updated: 0, offers_unchanged: 1 });
});
...
test("batch offer reporting supports mixed actions and preserves global totals", () => {
  ...
  assert.deepEqual(counts.existing_canonical_additions, { offers_created: 1, offers_updated: 0, offers_unchanged: 0 });
  assert.deepEqual(
    Object.fromEntries(Object.keys(counts.batch_1).map((key) => [key, counts.batch_1[key] + counts.batch_2[key] + counts.batch_3[key] + counts.batch_4[key] + counts.batch_5[key] + counts.existing_canonical_additions[key]])),
    { offers_created: 5, offers_updated: 9, offers_unchanged: 59 }
  );
});

test("batch offer reporting rejects unknown, missing, and duplicate slugs", () => {
  const valid = rowLevelActions(Array(10).fill("unchanged"), Array(12).fill("unchanged"), Array(16).fill("unchanged"), Array(14).fill("unchanged"), Array(20).fill("unchanged"), "create");
...
test("adapter report keeps existing batches and the canonical 508 addition separate", async () => {
  const rowLevelOffers = rowLevelActions(
    ...
    Array(20).fill("unchanged"),
    "create"
  );
  const output = [
    "new products would be created: 0",
    "retailer_products would be created: 1",
    "offers would be created: 1",
    "offers would be updated: 0",
    "offers unchanged: 72",
    "price_history rows would be created: 1",
  ...
  assert.deepEqual(result.report.batches.batch_5, {
    ...
    offers_created: 0, offers_updated: 0, offers_unchanged: 20,
    new_products_planned: 0,
    new_retailer_products_planned: 0,
    new_offers_planned: 0,
    new_price_history_rows_planned: 0,
  });
  assert.deepEqual(result.report.batches.existing_canonical_additions, {
    configured: 1, mapped: 1,
    canonical_product_id: 508,
    existing_canonical_mappings: 1,
    new_products_planned: 0,
    new_retailer_products_planned: 1,
    new_offers_planned: 1,
    new_price_history_rows_planned: 1,
